...
Framework-free TypeScript client for CoC REST and realtime APIs. It exposes domain clients for git, health, memory, models, preferences, processes, pull requests, queue, schedules, seen-state, skills, tasks, templates, wiki, work items, workspaces, and workflows, plus WebSocket events and per-process SSE streaming helpers.
