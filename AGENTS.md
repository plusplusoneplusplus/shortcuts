...
**Follow-up routing:** When a follow-up message arrives via `POST /api/processes/:id/message`, the handler routes based on task state: completed tasks use `requeueForFollowUp` (reuses the task ID); running/queued tasks buffer into `pendingMessages` on the process (avoiding duplicate task IDs); terminal-but-not-completed tasks (failed/cancelled) enqueue a fresh task. The `ProcessLifecycleRunner` drains one pending message after each task completion via `onDrainPendingMessages`, chaining follow-ups server-side. The client-side drain (`flushQueueRef` in `useSendMessage`) only processes `immediate` (steering) messages; `enqueue` messages are removed from the local queue after server persistence.
...
- `processes.db` — SQLite process store (default backend; schema version 4); also stores queue tasks, schedule runs, per-process seen/unseen state (`seen_at` column), commit-chat bindings, and per-process last-event timestamp (`last_event_at` column)
