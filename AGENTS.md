...
- `chat/` — Reusable conversation rendering: `ConversationTurnBubble`, `ConversationMiniMap`, `ConversationMetadataPopover`, tool call components (`ToolCallView`, `ToolCallGroupView`, `ToolResultPopover`, `WhisperCollapsedGroup`), `CommitStrip`, `JsonResponseView`, `NoteEditCard`, and utilities (`commitDetection`, `toolGroupUtils`, `timeline-utils`, `json-utils`, `chatConversationUtils`). Barrel: `chat/index.ts`.
...
- `shared/` — Generic UI primitives: `Button`, `Card`, `Dialog`, `Spinner`, `MarkdownView`, `CopySectionBtn`, `RichTextInput`, `ImageGallery`, `FilePathLink`, etc. Barrel: `shared/index.ts`.
