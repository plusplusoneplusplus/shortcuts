 ...
 * like progress bars, spinners, and color-coded output.
 *
...
/**
 * Simple progress display for pipeline execution
 */
export class ProgressDisplay {
    ...
    update(current: number, message?: string): void {
        ...
        const line = `${this.label}: ${parts.join(' ')}${suffix}`;

        if (process.stderr.isTTY) {
            process.stderr.write(`\r\x1b[K${cyan(SYMBOLS.spinner[0])} ${line}`);
        } else {
    ...
    complete(message?: string): void {
        const msg = message || `${this.label}: Done`;
        if (process.stderr.isTTY) {
            process.stderr.write(`\r\x1b[K${green(SYMBOLS.success)} ${msg}\n`);
        } else {
