...
describe('QueueClient', () => {
  it('calls queue list, stats, enqueue, and control endpoints', async () => {
    ...
    await client.releaseRepoGate({ repoId: 'repo/b' });
    ...
    await client.skipTaskDelay('all', { repoId: 'repo/b' });
    ...
    expect(adapter.calls.map(c => c.path)).toEqual([
      ...
      '/queue/pause',
      '/queue/pause',
      '/queue/repo-gate/release',
      '/queue/pause-autopilot',
      '/queue/task-delay',
      '/queue/task-delay/skip',
      '/queue/pause-marker',
      '/queue/task%2F1',
    ...
    expect(adapter.calls[9].options?.body).toEqual({ durationHours: 3 });
    expect(adapter.calls[10].options?.query).toEqual({ repoId: 'repo/b' });
    expect(adapter.calls[10].options?.body).toEqual({ scope: 'autopilot', delayMinutes: 15 });
    expect(adapter.calls[11].options?.query).toEqual({ repoId: 'repo/b' });
    expect(adapter.calls[11].options?.body).toEqual({ scope: 'all' });
    expect(adapter.calls[12].options?.body).toEqual({ afterIndex: 0, repoId: 'repo/a', durationHours: 2 });
