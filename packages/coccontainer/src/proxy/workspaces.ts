 ...
 */
export async function fetchAgentWorkspaces(agentAddress: string, headers?: Record<string, string>): Promise<RemoteWorkspace[]> {
    try {
        const result = await proxyRequest(agentAddress, 'GET', '/api/workspaces', undefined, headers);
