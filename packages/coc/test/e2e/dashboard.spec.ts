...
test.describe('Dashboard — Processes tab', () => {
    ...
    test('type filter narrows the process list', async ({ page, serverUrl }) => {
        await seedProcess(serverUrl, 'type-clar', { type: 'clarification', promptPreview: 'Clar' });
        await seedProcess(serverUrl, 'type-cr', { type: 'code-review', promptPreview: 'CR' });
        await page.goto(serverUrl);

        await expect(page.locator('.process-item')).toHaveCount(2, { timeout: 5000 });

        await page.selectOption('#type-filter', 'clarification');
        await expect(page.locator('.process-item')).toHaveCount(1);
        await expect(page.locator('.process-item')).toContainText('Clar');
    });

    test('processes are grouped by status', async ({ page, serverUrl }) => {
        await seedProcess(serverUrl, 'grp-running', { status: 'running' });
        await seedProcess(serverUrl, 'grp-completed', { status: 'completed' });
        await page.goto(serverUrl);
        ...
        const headers = page.locator('.status-group-header');
