...
/**
 * Seed an exclusive (autopilot-mode) chat task.
 * Autopilot mode routes through the exclusive queue (max 1 concurrent),
 * which is what makes drain-order testing deterministic.
 */
...
/** Navigate to a queue task's process detail page. */
...
test.describe('Queue Drain Behavior', () => {
    test('multiple tasks drain in FIFO order', async ({ serverUrl, mockAI }) => {
        ...
        const t1 = await seedExclusiveTask(serverUrl, 'Task 1');
        const t2 = await seedExclusiveTask(serverUrl, 'Task 2');
        const t3 = await seedExclusiveTask(serverUrl, 'Task 3');
    ...
    test('slow AI streaming does not break the drain loop', async ({ serverUrl, mockAI }) => {
        ...
        const t1 = await seedExclusiveTask(serverUrl, 'Slow task 1');
        const t2 = await seedExclusiveTask(serverUrl, 'Slow task 2');
    ...
    test('follow-up message is buffered while running and routed after completion', async ({
        ...
        const t1 = await seedExclusiveTask(serverUrl, 'Initial task');
        await waitForTaskStatus(serverUrl, t1.id as string, ['running']);
    ...
    test('failed task does not block the queue', async ({ serverUrl, mockAI }) => {
        ...
        const t1 = await seedExclusiveTask(serverUrl, 'Failing task');
        const t2 = await seedExclusiveTask(serverUrl, 'Should still run');
    ...
    test('UI updates when queued task starts running', async ({
        ...
        const t1 = await seedExclusiveTask(serverUrl, 'Blocking task');
        const t2 = await seedExclusiveTask(serverUrl, 'Queued task');

        await waitForTaskStatus(serverUrl, t1.id as string, ['running']);
        expect(await getTaskStatus(serverUrl, t2.id as string)).toBe('queued');

        // Navigate to task 2's page → should show PendingTaskInfoPanel
        await gotoQueueTask(page, serverUrl, t2.id as string);
    ...
    test('SSE delivers streaming chunks for queued-then-running task', async ({
        ...
        const t1 = await seedExclusiveTask(serverUrl, 'Block for SSE');
        const t2 = await seedExclusiveTask(serverUrl, 'SSE task');
