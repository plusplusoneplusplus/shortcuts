 ...
 *   remoteShell swaps the repo tab strip for the remote-first header). The
 *   existing E2E suite is written against the classic shell, so a resolved
 *   config with these on makes shared helpers (e.g. navigateToGitTab clicking
 *   `.repo-sub-tab[data-subtab="git"]`) hang until the test times out. Pin both
 *   off so the suite exercises the layout it targets.
 */
