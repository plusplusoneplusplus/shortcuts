...
describe('execute() — deliveryMode forwarding via chat-followup', () => {
    ...
    it('forwards deliveryMode from task payload to executeFollowUp', async () => {
        ...
        expect(spy).toHaveBeenCalledWith('proc-1', 'forwarded', undefined, undefined, 'immediate', undefined);
        spy.mockRestore();
    });
    ...
    it('passes undefined deliveryMode when not set on payload', async () => {
        ...
        expect(spy).toHaveBeenCalledWith('proc-1', 'no mode', undefined, undefined, undefined, undefined);
        spy.mockRestore();
    });
});
