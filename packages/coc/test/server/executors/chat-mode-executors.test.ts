...
import { CHAT_STYLE_SYSTEM_TAG, buildChatStyleSystemMessage } from '../../../src/server/executors/chat-style';
...
// ============================================================================
// Chat style — provider boundary
// ============================================================================

describe('Chat style reaches the provider boundary', () => {
    let store: ReturnType<typeof createMockProcessStore>;

    function styleTask(chatStyle: unknown, mode: 'ask' | 'autopilot' = 'ask', id = 'task-style'): QueuedTask {
        return {
            id,
            type: 'chat',
            priority: 'normal',
            status: 'running',
            createdAt: Date.now(),
            payload: { kind: 'chat', mode, prompt: 'Hello', chatStyle },
            config: {},
            displayName: 'Hello',
        } as unknown as QueuedTask;
    }
    ...
    it.each(['copilot', 'codex', 'claude'] as const)(
        'sends the same provider-neutral style block for %s',
        async (provider) => {
            const executor = new ChatExecutor(store, makeOptions(store, {
                provider,
                getChatStyleSelectorEnabled: () => true,
            }));

            await executor.execute(styleTask('direct', 'ask', `task-style-${provider}`), 'Hello');

            const call = sdkMocks.mockSendMessage.mock.calls[0][0];
            expect(call.systemMessage?.content).toContain(buildChatStyleSystemMessage('direct', true));
        },
    );

    it('places the style block after repo behavior instructions and before source-link formatting', async () => {
        const executor = new ChatExecutor(store, makeOptions(store, { getChatStyleSelectorEnabled: () => true }));

        await executor.execute(styleTask('analytical'), 'Hello');

        const content = sdkMocks.mockSendMessage.mock.calls[0][0].systemMessage!.content as string;
        expect(content.indexOf(READ_ONLY_SYSTEM_MESSAGE)).toBeLessThan(content.indexOf(`<${CHAT_STYLE_SYSTEM_TAG}>`));
        expect(content.indexOf(`</${CHAT_STYLE_SYSTEM_TAG}>`))
            .toBeLessThan(content.indexOf(SOURCE_LOCATION_MARKDOWN_LINK_SYSTEM_MESSAGE));
    });

    it('includes the block exactly once', async () => {
        const executor = new ChatExecutor(store, makeOptions(store, { getChatStyleSelectorEnabled: () => true }));

        await executor.execute(styleTask('human'), 'Hello');

        const content = sdkMocks.mockSendMessage.mock.calls[0][0].systemMessage!.content as string;
        expect(content.match(new RegExp(`<${CHAT_STYLE_SYSTEM_TAG}>`, 'g'))).toHaveLength(1);
    });

    it('applies the style to Autopilot chats too', async () => {
        const executor = new AutopilotExecutor(store, makeOptions(store, { getChatStyleSelectorEnabled: () => true }));

        await executor.execute(styleTask('structured', 'autopilot', 'task-style-auto'), 'Hello');

        const content = sdkMocks.mockSendMessage.mock.calls[0][0].systemMessage!.content as string;
        expect(content).toContain(`<${CHAT_STYLE_SYSTEM_TAG}>`);
        expect(content).toContain('Selected style: Structured.');
    });

    it('injects nothing when the admin flag is off, even though the payload carries a style', async () => {
        const executor = new ChatExecutor(store, makeOptions(store, { getChatStyleSelectorEnabled: () => false }));

        await executor.execute(styleTask('direct'), 'Hello');

        const content = sdkMocks.mockSendMessage.mock.calls[0][0].systemMessage?.content ?? '';
        expect(content).not.toContain(CHAT_STYLE_SYSTEM_TAG);
    });

    it('injects nothing when the gate callback is not wired at all', async () => {
        const executor = new ChatExecutor(store, makeOptions(store));
    ...
    it('injects nothing for a legacy payload that omits the style', async () => {
        const executor = new ChatExecutor(store, makeOptions(store, { getChatStyleSelectorEnabled: () => true }));

        await executor.execute(makeChatTask('ask', 'task-style-legacy'), 'Hello');

        const content = sdkMocks.mockSendMessage.mock.calls[0][0].systemMessage?.content ?? '';
        expect(content).not.toContain(CHAT_STYLE_SYSTEM_TAG);
    });

    it('leaves excluded background jobs (classification) untouched', async () => {
        const executor = new ClassificationExecutor(store, makeOptions(store, { getChatStyleSelectorEnabled: () => true }));

        await executor.execute(makeClassificationTask('task-style-classify', { chatStyle: 'direct' }), 'Classify PR #42');
