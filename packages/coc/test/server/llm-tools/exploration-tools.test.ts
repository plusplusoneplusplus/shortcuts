...
import { createExplorationTools, KUSTO_QUERY_ROW_SAMPLE } from '../../../src/server/llm-tools/exploration-tools';
import {
...
describe('kusto_query tool', () => {
    ...
    beforeEach(() => {
        dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coc-exploration-tool-'));
    ...
    it('creates a new exploration, runs the query, and returns schema + rows + embed', async () => {
        ...
        const reloaded = parseExplorationState(store.getCanvas(WS, res.canvasId)!.content);
    ...
    it('updates an existing exploration when canvasId is provided', async () => {
        const seed = store.createCanvas({
            workspaceId: WS,
            title: 'Existing',
            type: 'exploration',
            content: serializeExplorationState(createEmptyExplorationState({
        ...
        const reloaded = parseExplorationState(store.getCanvas(WS, seed.id)!.content);
    ...
    it('surfaces a query/auth error as success:false but still persists the exploration with an embed', async () => {
    ...
    it('rejects a canvasId that is not an exploration', async () => {
        ...
        expect(res.success).toBe(false);
        expect(res.error).toMatch(/not an exploration/);
    });
});
