...
describe('browseDirectory', () => {
    it('returns null for a non-existent path', () => {
        expect(browseDirectory(path.join(tmpDir, 'nope'))).toBeNull();
    });

    it('returns null for a file path', () => {
        const f = path.join(tmpDir, 'file.txt');
        fs.writeFileSync(f, 'hi');
        expect(browseDirectory(f)).toBeNull();
    });

    it('lists plain subdirectories', () => {
    ...
    it('detects git repos via .git directory', () => {
    ...
    it('hides dot-prefixed directories by default', () => {
    ...
    it('shows dot-prefixed directories when showHidden=true', () => {
        fs.mkdirSync(path.join(tmpDir, '.hidden'));
        fs.mkdirSync(path.join(tmpDir, 'visible'));

        const result = browseDirectory(tmpDir, true)!;
    ...
    it('returns parent path for non-root directories', () => {
        const child = path.join(tmpDir, 'child');
        fs.mkdirSync(child);

        const result = browseDirectory(child)!;
    ...
    it('excludes symlinks that point to files', () => {
    ...
    it('gracefully skips broken symlinks', () => {
