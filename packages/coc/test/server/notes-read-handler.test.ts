 ...
 *
 * Tests for GET /api/workspaces/:id/notes/content security boundary:
...
describe('Notes Read Handler — GET /notes/content security', { timeout: 30_000 }, () => {
    ...
    it('returns 200 for a file inside the notes directory (relative path)', async () => {
        ...
        const res = await request(contentUrl(srv, 'notes/hello.md'));
    ...
    it('returns 200 for any .md file inside the workspace data directory (relative path)', async () => {
        const srv = await startServer();
        await registerWorkspace(srv);
        writeFile('tasks/notes.md', '# Notes');

        const res = await request(contentUrl(srv, 'tasks/notes.md'));
        expect(res.status).toBe(200);
    });
