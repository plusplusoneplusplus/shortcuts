...
describe('normalizeFollowUpInput – chatStyle', () => {
    it.each(['human', 'direct', 'analytical', 'structured'])('accepts the stable wire value %s', (style) => {
        const result = normalizeFollowUpInput({ chatStyle: style }, 'copilot');
        expect(result.ok).toBe(true);
        expect(result.ok && result.value.chatStyle).toBe(style);
    });

    it.each([
        ['an unknown style', 'friendly'],
        ['a label instead of a wire value', 'Direct'],
        ['an empty string', ''],
        ['a non-string', 7],
    ])('rejects %s so the user never silently gets a different style', (_label, style) => {
        const result = normalizeFollowUpInput({ chatStyle: style }, 'copilot');
        expect(result.ok).toBe(false);
        expect(!result.ok && result.error).toMatch(/invalid chatStyle/i);
        expect(!result.ok && result.error).toContain('human, direct, analytical, structured');
    });

    it('omits chatStyle for a legacy client that does not send it', () => {
        const result = normalizeFollowUpInput({}, 'copilot');
        expect(result.ok).toBe(true);
        expect(result.ok && result.value.chatStyle).toBeUndefined();
    });

    it('treats an explicit null as absent rather than invalid', () => {
        const result = normalizeFollowUpInput({ chatStyle: null }, 'copilot');
        expect(result.ok).toBe(true);
        expect(result.ok && result.value.chatStyle).toBeUndefined();
    });
});

describe('ProcessMessageDeliveryService – chat style delivery', () => {
    function runningBridge() {
        return {
            enqueue: vi.fn(),
    ...
    it('steers an immediate message that carries the style the conversation already has', async () => {
        const store = makeStore();
        const bridge = runningBridge();
        const service = makeService(store, bridge);

        const result = await service.deliver(
            makeProc({ status: 'running', metadata: { chatStyle: 'direct' } as never }),
            makeInput({ deliveryMode: 'immediate', chatStyle: 'direct' }),
        );

        expect(result.path).toBe('steered');
        expect(bridge.steerProcess).toHaveBeenCalled();
        expect(store.pending).toHaveLength(0);
    });

    it('steers an immediate message that carries no style at all', async () => {
        const store = makeStore();
        const bridge = runningBridge();
        const service = makeService(store, bridge);
        ...
        const result = await service.deliver(
            makeProc({ status: 'running', metadata: { chatStyle: 'direct' } as never }),
            makeInput({ deliveryMode: 'immediate' }),
        );

        expect(result.path).toBe('steered');
    });

    it('buffers a style-changing immediate message instead of steering it', async () => {
        // Steering can only inject user text; it cannot rebuild the in-flight
        // response's system message, so the new style must run as the next turn.
        const store = makeStore();
        const bridge = runningBridge();
        const service = makeService(store, bridge);
        ...
        const result = await service.deliver(
            makeProc({ status: 'running', metadata: { chatStyle: 'direct' } as never }),
            makeInput({ deliveryMode: 'immediate', chatStyle: 'structured' }),
        );

        expect(result.path).toBe('buffered');
        expect(bridge.steerProcess).not.toHaveBeenCalled();
        expect(store.pending[0]).toMatchObject({ chatStyle: 'structured' });
    });

    it('buffers when a legacy conversation with no stored style receives one', async () => {
        const store = makeStore();
        const bridge = runningBridge();
        const service = makeService(store, bridge);

        const result = await service.deliver(
            makeProc({ status: 'running', metadata: {} as never }),
            makeInput({ deliveryMode: 'immediate', chatStyle: 'human' }),
        );

        expect(result.path).toBe('buffered');
        expect(store.pending[0]).toMatchObject({ chatStyle: 'human' });
    });

    it('omits chatStyle from the pending message when none was selected', async () => {
        const store = makeStore();
        const bridge = {
            enqueue: vi.fn(),
            findTaskByProcessId: vi.fn().mockReturnValue({ id: 't1', type: 'chat', status: 'running' }),
        ...
        expect(store.pending[0]).not.toHaveProperty('chatStyle');
    });

    it('carries chatStyle onto the enqueued payload for a terminal conversation', async () => {
        const store = makeStore();
        const bridge = { enqueue: vi.fn(), findTaskByProcessId: vi.fn().mockReturnValue(undefined) };
        const service = makeService(store, bridge);

        const result = await service.deliver(
            makeProc({ status: 'completed' }),
            makeInput({ chatStyle: 'analytical' }),
        );

        expect(result.path).toBe('enqueued');
        expect(bridge.enqueue.mock.calls[0][0].payload).toMatchObject({ chatStyle: 'analytical' });
    });

    it('passes chatStyle to executeFollowUp on the direct-execute path', async () => {
        const store = makeStore();
        const bridge = { executeFollowUp: vi.fn().mockResolvedValue(undefined) };
        const service = makeService(store, bridge);

        await service.deliver(makeProc(), makeInput({ chatStyle: 'human' }));

        expect(bridge.executeFollowUp.mock.calls[0][11]).toBe('human');
    });
});
