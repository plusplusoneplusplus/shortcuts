...
describe('Queue Pause Markers', () => {
    ...
    async function insertMarker(afterIndex: number) {
        const res = await post(`${server!.url}/api/queue/pause-marker`, { afterIndex });
    ...
    // ========================================================================
    // Persistence: pause markers NOT persisted across restart
    // ========================================================================

    it('pause marker is NOT present after server restart (not persisted)', async () => {
        await enqueueTask('T1');
        const markerId = await insertMarker(0);

        // Verify marker is present before restart
        const before = await request(`${server!.url}/api/queue`);
        expect(JSON.parse(before.body).queued.some((i: any) => i.kind === 'pause-marker')).toBe(true);
