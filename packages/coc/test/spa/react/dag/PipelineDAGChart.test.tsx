...
describe('WorkflowDAGChart — node interaction', () => {
    it('clicking a node sets it as selected', () => {
        render(<WorkflowDAGChart data={makeData()} isDark={false} phaseDetails={makePhaseDetails()} />);
    ...
    it('renders WorkflowPhasePopover when a node is selected', () => {
        render(<WorkflowDAGChart data={makeData()} isDark={false} phaseDetails={makePhaseDetails()} />);
    ...
    it('does not render WorkflowPhasePopover when no node is selected', () => {
        render(<WorkflowDAGChart data={makeData()} isDark={false} phaseDetails={makePhaseDetails()} />);
        expect(screen.queryByTestId('phase-popover')).toBeNull();
    });
    ...
    it('clicking outside the chart clears selection', () => {
        render(
            <div>
                ...
                <WorkflowDAGChart data={makeData()} isDark={false} phaseDetails={makePhaseDetails()} />
            </div>
        );
        fireEvent.click(screen.getByTestId('dag-node-input'));
        expect(screen.getByTestId('phase-popover')).toBeDefined();
    ...
    it('forwards onNodeClick callback', () => {
        ...
        render(<WorkflowDAGChart data={makeData()} isDark={false} onNodeClick={onNodeClick} />);
    ...
    it('passes onScrollToConversation for failed phases', () => {
        ...
        render(<WorkflowDAGChart data={data} isDark={false} phaseDetails={details} onScrollToConversation={onScroll} />);
        fireEvent.click(screen.getByTestId('dag-node-map'));
...
describe('WorkflowDAGChart — visual context layer', () => {
    ...
    it('breadcrumb step count matches node count', () => {
        const data = makeData();
        render(<WorkflowDAGChart data={data} isDark={false} />);
    ...
    it('parallelCount prop causes parallel badge on map node', () => {
        render(<WorkflowDAGChart data={makeData()} isDark={false} parallelCount={4} />);
    ...
    it('parallel badge only appears on map node, not others', () => {
        render(<WorkflowDAGChart data={makeData()} isDark={false} parallelCount={3} />);
...
describe('WorkflowDAGChart — hover tooltip', () => {
    ...
    it('renders hover tooltip when pipelineConfig is provided and a node is hovered', () => {
        ...
        render(<WorkflowDAGChart data={makeData()} isDark={false} pipelineConfig={makePipelineConfig()} />);
    ...
    it('hover tooltip disappears when selectedPhase is set (click takes precedence)', () => {
        render(<WorkflowDAGChart data={makeData()} isDark={false} pipelineConfig={makePipelineConfig()} phaseDetails={makePhaseDetails()} />);
        // Hover first
...
describe('WorkflowDAGChart — edge annotations', () => {
    it('renders edge labels when pipelineConfig is provided with input data', () => {
        ...
        render(<WorkflowDAGChart data={makeData()} isDark={false} pipelineConfig={config} />);
    ...
    it('renders "filtered" badge when filter phase is present', () => {
        ...
        render(<WorkflowDAGChart data={data} isDark={false} pipelineConfig={config} />);
        const edgeLabels = screen.getAllByTestId('dag-edge-label');
        expect(edgeLabels.some(el => el.textContent?.includes('filtered'))).toBe(true);
    });
});

describe('WorkflowDAGChart — validation error pins', () => {
    ...
    it('no error pins when validationErrors is empty array', () => {
        render(<WorkflowDAGChart data={makeData()} isDark={false} validationErrors={[]} />);
        expect(screen.queryAllByTestId('dag-error-pin')).toHaveLength(0);
    });
});
...
describe('WorkflowDAGChart — preview mode', () => {
    it('does not render legend when previewMode is true', () => {
        render(<WorkflowDAGChart data={makeData()} isDark={false} previewMode />);
