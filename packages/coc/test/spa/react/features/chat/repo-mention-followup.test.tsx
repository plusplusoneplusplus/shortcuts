 ...
 *
 * Composer integration for `#repo_name` mentions in `FollowUpInputArea`
 * (AC-01..03) — the second composer surface, alongside
 * `repo-mention-composer.test.tsx` which covers `NewChatArea`.
 *
 * Beyond the same trigger/filter/insert contract, this surface is controlled:
 * the parent owns `followUpInput`, and a value-sync effect pushes programmatic
 * changes back into the editor. The insert must set `skipNextSyncRef` so that
 ...
 * text is already in the DOM by the time the value-sync effect runs — otherwise
 * that effect would echo every keystroke back through `setValue`.
 */
...
describe('FollowUpInputArea #repo mentions', () => {
    ...
    it('resolves the group from the task metadata when no workspaceId prop is passed', () => {
        renderFollowUp({ task: { id: 'task-1', metadata: { workspaceId: 'group-demo' } } as any });
        type('#');
        expect(screen.getByTestId('repo-mention-menu')).toBeTruthy();
    });
    ...
    it('does nothing new outside a repo-group workspace', () => {
        renderFollowUp({ workspaceId: 'ws-1' });
        type('#');
        expect(screen.queryByTestId('repo-mention-menu')).toBeNull();
    });
    ...
    it('inserts plain "#name " and places the caret after the space on Enter', () => {
        ...
        renderFollowUp({ workspaceId: 'group-demo', onSend });
        type('look at #be');
        fireEvent.keyDown(editor(), { key: 'Enter' });

        expect(tracker.calls).toEqual([['look at #beta ', 'look at #beta '.length]]);
        // Enter was consumed by the menu, not treated as send.
    ...
    it('keeps ghost-text Tab behind the repo menu but working once it closes', () => {
        mockAutocomplete.completion = ' rest';
        mockAutocomplete.accept.mockReturnValue('#alpha rest');
        renderFollowUp({ workspaceId: 'group-demo' });
        type('#al');

        // Menu open: Tab accepts the highlighted member, not the ghost text.
        fireEvent.keyDown(editor(), { key: 'Tab' });
        expect(mockAutocomplete.accept).not.toHaveBeenCalled();
        expect(tracker.calls.at(-1)).toEqual(['#alpha ', '#alpha '.length]);

        // Menu closed: Tab falls through to the ghost-text accept as before.
        fireEvent.keyDown(editor(), { key: 'Tab' });
