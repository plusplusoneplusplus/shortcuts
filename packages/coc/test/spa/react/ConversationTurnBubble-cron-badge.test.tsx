/**
 * Tests for ConversationTurnBubble turnSource badge — verifies cron/wakeup badges
...
describe('ConversationTurnBubble turnSource badge', () => {
    it('renders cron badge when turnSource.source is cron', () => {
        const { container } = render(
            <ConversationTurnBubble
                turn={makeTurn({
                    turnSource: { source: 'cron', cronId: 'cron-123' },
                })}
            />,
        );
        const badge = container.querySelector('[data-testid="turn-source-badge"]');
        expect(badge).toBeTruthy();
        expect(badge?.textContent).toContain('cron');
        expect(badge?.querySelector('[data-testid="cron-icon"]')).toBeTruthy();
        expect(badge?.getAttribute('title')).toContain('Cron tick');
        expect(badge?.getAttribute('title')).toContain('cron-123');
    });
    ...
    it('does not render turnSource badge on assistant turns', () => {
        const { container } = render(
            <ConversationTurnBubble
                turn={makeTurn({
                    role: 'assistant',
                    turnSource: { source: 'cron', cronId: 'cron-1' },
                })}
            />,
        );
