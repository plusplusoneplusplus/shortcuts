...
describe('useChatSSE', () => {
    ...
    describe('note-file-edit event', () => {
        it('calls onNoteFileEdit when note-file-edit SSE event is received', () => {
            const onNoteFileEdit = vi.fn();
            renderHook(() => useChatSSE(makeOptions({ onNoteFileEdit })));
            act(() => {
                // Server sends event.noteFileEdit directly (unwrapped from the process event)
                MockEventSource.last._emit('note-file-edit', {
                    toolCallId: 'tc-1',
            ...
            expect(onNoteFileEdit).toHaveBeenCalledOnce();
            expect(onNoteFileEdit).toHaveBeenCalledWith({
                toolCallId: 'tc-1',
                filePath: '/notes/my-note.md',
                oldStr: 'old text',
                newStr: 'new text',
        ...
        it('does not throw when onNoteFileEdit is not provided', () => {
        ...
        it('calls onNoteFileEdit with all payload fields', () => {
            const onNoteFileEdit = vi.fn();
            renderHook(() => useChatSSE(makeOptions({ onNoteFileEdit })));
            const payload = { toolCallId: 'tc-3', filePath: '/foo.md', oldStr: 'x', newStr: 'y' };
            act(() => {
                MockEventSource.last._emit('note-file-edit', payload);
            });
            expect(onNoteFileEdit).toHaveBeenCalledWith(payload);
        });
    });
});
