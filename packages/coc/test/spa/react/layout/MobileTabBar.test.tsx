...
describe('MobileTabBar: active tab highlighting', () => {
    it('highlights active pinned tab in blue', () => {
        ...
        const chatsBtn = screen.getByTestId('mobile-tab-bar').querySelector('[data-tab="chats"]') as HTMLElement;
        expect(chatsBtn.className).toContain('text-[#0078d4]');
    });
    ...
    it('active pinned tab has background tint', () => {
        renderBar({ activeTab: 'chats' });
        const chatsBtn = screen.getByTestId('mobile-tab-bar').querySelector('[data-tab="chats"]') as HTMLElement;
        expect(chatsBtn.className).toContain('bg-[#0078d4]/10');
        expect(chatsBtn.className).toContain('rounded-lg');
    });
    ...
    it('inactive pinned tab has no background tint', () => {
        ...
        expect(workItemsBtn.className).not.toContain('bg-[#0078d4]/10');
    });
...
describe('MobileTabBar: More sheet', () => {
    ...
    it('selecting a tab from the sheet calls onTabChange', () => {
        ...
        fireEvent.click(screen.getByTestId('mobile-tab-more-btn'));
        fireEvent.click(screen.getByTestId('mobile-tab-more-item-explorer'));
    ...
    it('active "more" tab is highlighted in the sheet', () => {
        renderBar({ activeTab: 'explorer' });
        fireEvent.click(screen.getByTestId('mobile-tab-more-btn'));
        const explorerBtn = screen.getByTestId('mobile-tab-more-item-explorer');
        expect(explorerBtn.className).toContain('text-[#0078d4]');
    });
    ...
    it('inactive "more" tabs are not highlighted in the sheet', () => {
        ...
        fireEvent.click(screen.getByTestId('mobile-tab-more-btn'));
        const explorerBtn = screen.getByTestId('mobile-tab-more-item-explorer');
        expect(explorerBtn.className).not.toContain('text-[#0078d4]');
    });
...
describe('MobileTabBar: badge display', () => {
    ...
    it('shows work-items badge when workItemCount > 0', () => {
        renderBar({ workItemCount: 5 });
        expect(screen.getByTestId('mobile-tab-badge-work-items')).toBeTruthy();
        expect(screen.getByTestId('mobile-tab-badge-work-items').textContent).toBe('5');
    });
    ...
    it('multiple badges can appear simultaneously', () => {
        renderBar({ activityCount: 5, workItemCount: 3 });
...
describe('MobileTabBar: custom pinned tabs', () => {
    it('respects custom pinnedTabs prop', () => {
        renderBar({ pinnedTabs: ['explorer', 'git', 'tasks'] });
        const nav = screen.getByTestId('mobile-tab-bar');
        expect(nav.querySelector('[data-tab="explorer"]')).toBeTruthy();
    ...
    it('more sheet shows non-pinned tabs when custom pinnedTabs provided', () => {
        renderBar({ pinnedTabs: ['explorer', 'git', 'tasks'], activeTab: 'explorer' });
        fireEvent.click(screen.getByTestId('mobile-tab-more-btn'));
        expect(screen.getByTestId('mobile-tab-more-item-chats')).toBeTruthy();
        expect(screen.queryByTestId('mobile-tab-more-item-explorer')).toBeNull();
    });
});
