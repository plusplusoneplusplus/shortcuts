/**
 * Tests for useQueueActivity hook — hook-level integration with context providers.
 */
...
describe('useQueueActivity', () => {
    it('returns empty fileMap and folderMap when no workspace matches', () => {
        const { result } = renderHook(() => useQueueActivity('nonexistent-ws'), { wrapper: Wrapper });
    ...
    it('returns empty maps when workspace exists but queue is empty', () => {
        ...
        const { result } = renderHook(() => useQueueActivity('any-ws'), { wrapper: Wrapper });
    ...
    it('returns stable references when called with same wsId and no queue change', () => {
        const { result, rerender } = renderHook(() => useQueueActivity('ws-1'), { wrapper: Wrapper });
