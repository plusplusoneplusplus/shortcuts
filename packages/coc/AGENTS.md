...
Capture-mode memory writes append raw candidates only. Aggregation runs through the manual repo memory API/UI action or explicit `memory-aggregate` queue tasks; chat-time `memory.add` tool completions must not auto-enqueue aggregation.
