...
Capture-mode memory writes upsert durable candidate rows in the repo/system memory database; duplicate normalized facts strengthen the same candidate via signal counts and provenance. Aggregation runs through the manual repo memory API/UI action or explicit `memory-aggregate` queue tasks; chat-time `memory.add` tool completions must not auto-enqueue aggregation. `memory-aggregate` queue tasks do not invoke AI or rewrite `MEMORY.md`; they inspect pending candidates and leave them pending until explicit promotion/drop/ignore decisions are implemented. Direct bounded memory tool/admin actions are the mutation paths for `MEMORY.md`.
