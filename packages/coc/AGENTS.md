...
Capture-mode memory writes append raw candidates only. Aggregation runs through the manual repo memory API/UI action or explicit `memory-aggregate` queue tasks; chat-time `memory.add` tool completions must not auto-enqueue aggregation. `memory-aggregate` queue tasks do not invoke AI or rewrite `MEMORY.md`; they drain pending raw candidates as dropped. Direct bounded memory tool/admin actions are the mutation paths for `MEMORY.md`.
