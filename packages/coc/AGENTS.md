...
Admin → Settings → Appearance & Navigation exposes global display preferences backed by `/api/preferences`, including inline HTML previews (`htmlEmbed.enabled`). The dashboard top bar uses a fixed code-defined utility icon order. When inline HTML previews are enabled, local `.html`/`.htm` markdown references in chat using image syntax `![alt](page.html)` render as sandboxed iframe previews through `/api/workspaces/:id/files/html` (link syntax stays a plain anchor). Default embed height is 600px, persisted per file via localStorage on user resize.
