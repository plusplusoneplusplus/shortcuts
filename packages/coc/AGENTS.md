...
Admin → Settings → Appearance & Navigation exposes global display preferences backed by `/api/preferences`, including inline HTML previews (`htmlEmbed.enabled`) and the top-bar customize entry. The dashboard top bar stores reorderable utility destination order in `GlobalPreferences.topBarItemOrder`, preserves hidden/feature-flagged item positions, ignores unknown IDs, and lets users reorder by desktop drag, keyboard drag, or the mobile reorder sheet. When inline HTML previews are enabled, local `.html`/`.htm` markdown references in chat using image syntax `![alt](page.html)` render as sandboxed iframe previews through `/api/workspaces/:id/files/html` (link syntax stays a plain anchor). Default embed height is 600px, persisted per file via localStorage on user resize.
Repo Settings → Notes exposes normal repository notes git auto-commit controls only. Virtual workspaces (`my_work`, `my_life`) omit the Notes settings sidebar item and route Notes settings deep links to Info.
