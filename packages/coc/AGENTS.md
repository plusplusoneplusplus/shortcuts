...
Admin → Settings → Appearance & Navigation exposes global display preferences backed by `/api/preferences`, including inline HTML previews (`htmlEmbed.enabled`) and inline ghost-text autocomplete (`promptAutocomplete.enabled`, with AI generation enabled by default via `promptAutocomplete.ai.enabled` and a configurable `promptAutocomplete.ai.model` — see `docs/wiki/prompt-autocomplete.md`). The dashboard top bar's right-hand action cluster is `[Connected pill | NotificationBell | Tools ▾ | Admin | Theme]` with a fixed code-defined order. Skills, Logs, Usage, Models, and Servers (when enabled) live as menu rows inside the **Tools** popover (`#tools-toggle` opens it, `#tools-popover` is the menu). Each row keeps its original DOM id (`skills-toggle`, `logs-toggle`, `stats-toggle`, `models-toggle`, `servers-toggle`), `aria-label`, `title`, and `data-tab` attribute so existing selectors still resolve once the popover is open. The WS status indicator renders as a rounded `Connected/Disconnected/...` pill on desktop (`[data-testid="ws-status-indicator"]` with a `data-ws-status` attribute and a `[data-testid="ws-status-label"]` child) and degrades to a bare colored dot on mobile (`[data-testid="ws-status-indicator-mobile"]`). The Tools button itself is hidden on mobile; mobile callers reach the same destinations through `BottomNav`. The repo Workflows tab and the New Schedule form's workflow action are gated by `workflows.enabled`. When inline HTML previews are enabled, local `.html`/`.htm` markdown references in chat using image syntax `![alt](page.html)` render as sandboxed iframe previews through `/api/workspaces/:id/files/html` (link syntax stays a plain anchor). Default HTML embed height is 600px, persisted per file via localStorage on user resize. Notes render allowlisted Google Maps links (`https://www.google.com/maps/embed?...`, `https://maps.google.com/maps?...`) as inline map iframes in both read mode and rich edit mode; `maps.app.goo.gl` share links remain plain anchors. Map embed height defaults to 400px and is persisted per URL via localStorage on user resize.
...
│               ├── features/chat/       # Chat conversation UI: ChatDetail/ChatListPane. `conversation/ConversationTurnBubble` renders user turns as right-aligned soft-gray rounded bubbles (`rounded-2xl bg-[#f3f4f6]`, amber when pinned) with a hover-revealed action meta strip absolute-positioned above the bubble (`role-label` is `sr-only`), and assistant turns as left-aligned circular avatar (`C` for assistant, `$_` for scripts, error palette when `turn.isError`) + borderless flowing content. When `processType === 'run-script'` and the body matches the `formatScriptResponse` shape (`**Script:** …`, `**Status:** …`, fenced `**stdout:**`/`**stderr:**`), `conversation/scriptOutputParser.ts` extracts script/cwd/exitCode/duration/stdout/stderr and `conversation/ScriptTerminalBlock.tsx` renders a dark `#1e1e1e` terminal window (window-style title bar with three `#3c3c3c` dots + script command label + cwd suffix, then a `<pre>` body with PASS=#4ec9b0 / FAIL=#f48771 token highlighting and dimmed Jest-style summary headers) instead of plain markdown; the bubble header gains an `exit N` / `timed out` suffix coloured by status (green=#15703a / red=#cf222e). `processType` is plumbed in from `ChatDetail` (`fullTask?.type ?? task?.type`) through `ConversationArea` and from `ItemConversationPanel` (`proc?.type`). Tool-call rendering inside `WhisperCollapsedGroup` expanded bodies adopts a compact "whisper-row" variant via `tool-calls/ToolCallVariant.tsx` (`ToolCallVariantProvider` value=`"whisper-row"`): each `ToolCallView` renders as a single flat row (`kind` pill + truncated path/summary + result metric + duration + chevron) with white surface + thin bottom border, color-coded pills per `tool-calls/toolKindUtils.ts` (Read/blue, Grep/Glob/green, Edit/Write/amber, Shell/PS/SQL/purple, Agent/Done/etc.), and the wrapping `ToolCallGroupView` adopts the surface card design (`bg-[#fafafa]`, `border-[#e5e7eb]`, `rounded-md`, no category emoji, `Show`/`Hide` blue toggle, `· ` font-mono duration prefix); the whisper collapsed-strip header and its hover popovers (skill/commit/file/memory) are unchanged. `QueuedFollowUps` (in `features/chat/QueuedBubble.tsx`) renders queued/pending follow-ups as a compact section indented to align with the assistant avatar gutter (`ml-9`): a `Queued · N` mono-uppercase muted label, then one dashed-border surface card per item (`bg-[#fafafa]`/`#252526`, `border-dashed border-[#e5e7eb]/[#3c3c3c]`, single-line `truncate` with `title` tooltip, no clock emoji) plus an optional `✕` cancel button (hover error palette `#ffebe9`/`#cf222e`, `data-testid="queued-item-cancel"`, `stopPropagation` so clicks don't bubble). `ConversationArea` accepts `onCancelPendingMessage` and forwards it to the queue; `ChatDetail.handleCancelPendingMessage` optimistically removes the entry from local `pendingQueue` state and calls `client.processes.deletePendingMessage(processId, messageId)`, restoring on API failure. Assistant turns with `turn.isError` render a dedicated `error-strip` aside (`role="alert"`, `data-testid="error-strip"`) inside `chat-message-content` instead of the regular markdown body — red `⚠` icon + "Stream interrupted" red bold title (`data-testid="error-strip-title"`) + the turn content rendered as markdown detail (`data-testid="error-strip-detail"`, `<code>` styled with mono pill background) + a `↺ Retry this turn` button (still carrying `.bubble-retry-btn` + `data-testid="retry-turn-btn"` for backward-compat). The legacy `⚠ Error` meta-row text and the tiny inline `↺ Retry` chevron in the meta row are gone — the strip is the sole error indicator. The model-change divider in `ConversationArea` adopts the reference layout: `<div class="model-divider ml-9 mt-3.5 mb-2 flex items-center gap-3">` with the `model-divider-label` (`font-mono uppercase tracking-[0.1em] text-[10.5px]` muted, model name in `<strong>` foreground) FIRST and a single solid `model-divider-rule` (`flex-1 h-px bg-[#e5e7eb]/[#3c3c3c]`) on the right; the legacy 🤖 emoji and dashed double-rule layout are gone. (Activity branch renders an activity-compact action bar — `[+ New chat ⌘N (flex-1, primary)] [↺ refresh utility] [● ALL | ● AP]` split pause pill — followed by a 3-column scope segmented control — Chats / Automations / All — and the search box, all wrapped in a tight `flex flex-col gap-1.5` toolbar sub-container so the three rows share a 6px vertical rhythm instead of inheriting the looser parent `gap-2 md:gap-3` (per-row `mb-*` margins are removed to avoid double-spacing); the legacy `queue-filter-dropdown` type filter has been retired in favor of the scope segmented control, so `setExcludedTypes` / `availableFilters` and the `FilterDropdown` import are gone (server-managed `myWorkExcludedTypes` from `SET_WELCOME_PREFERENCES` is still applied read-only). The segmented buttons use `min-w-0` + `truncate` + `whitespace-nowrap` so narrow widths ellipsis the longest label ("Automations") rather than wrap the count. The split pause pill keeps BOTH "Pause All" and "Pause AP" toggles visible side-by-side: each shows a status dot + scope tag (ALL/AP) where the dot+label colour alone signals running (green) vs paused (amber); only the paused state reveals an inline "PAUSED" or remaining-time label, so the pill stays compact when idle. Click on a section drives the same handler as the legacy "⏸ All / ⏸ AP" buttons (open duration menu when running, resume immediately when paused). ⌘N / Ctrl+N triggers `onNewChat ?? onOpenDialog` when the activity pane is visible and the detail pane isn't focused. Counts are sourced from running+queued+history; selection persists in `localStorage['coc-activity-scope']`, defaulting to `'all'`). FollowUpInputArea + NewChatArea use a compact stacked layout. The input card holds the editor and a single mobile-responsive (`flex-wrap`) toolbar containing — in this order — the ModePillSelector (rectangular `rounded-md` segmented control: pills `px-2 py-[2px] text-[11px]` with 4px dots, active pill uses an inset border shadow `shadow-[inset_0_0_0_1px_#d0d0d0]` instead of an outer border, dot colors match `MODE_BORDER_COLORS`: ask=yellow, plan=blue, autopilot=green), the model picker chip (single source of truth for the active model — shows session model or override and an inline ✕ to clear; no separate override badge), uniform `ctool` buttons (slash, `@` mention-skill, attach — each `h-[22px] px-1.5 rounded-sm text-[11px]` with 11px icons), and the QueueFollowUpButton (`h-[24px] pl-2 pr-1.5 text-[11px]` with a `sm:`-gated ⌘↵ shortcut hint that uses a vertical `border-l` separator + `text-[9px]` glyph instead of a boxed kbd). The `@` mention-skill button shares the slash button's handler — both open the skill picker. Ultra-compact density: outer `py-2`, stack `space-y-1`, RichTextInput `min-h-[28px] px-3 pt-2 pb-1 text-[13.5px]`, toolbar `pl-2 pr-1.5 py-1` with `gap-y-0.5`. The card is the SOLE focus indicator: `chat-input-bar` carries `focus-within:ring-2` with a mode-coloured ring (`MODE_BORDER_COLORS[mode].ring` uses the `focus-within:` prefix at `/30` opacity so the ring colour propagates from the focused contenteditable child to the parent card). The inner contenteditable explicitly neutralizes the base RichTextInput chrome with `border-transparent focus:ring-transparent` so it adds no visible inner border or default-blue focus ring inside the card (other RichTextInput consumers — EnqueueDialog, ItemConversationPanel, NoteChatPanel, GenerateTaskDialog, CommitChatPanel — keep the base chrome). Legacy single-row layout is preserved when `compactModeSelector` is true (e.g., narrow side panels). SlashCommandMenu renders as a card popover with a header and `↵` indicator on the highlighted row. ChatHeader uses `ChatStatusPill` (replaces the legacy `Badge`) — a rounded-full bordered pill with a leading dot, the status label, and (in wide tier) inline duration; the dot pulses for `running`/`cancelling`. The right-side action cluster (scratchpad / float / popout / copy / HTML / PDF / select / metadata / overflow) uses two shared design tokens in `ChatHeader.tsx`: `ICON_BTN_CLASS` (26×26 flat icon button, `text-[#848484]` resting, `hover:bg-[#e8e8e8]` light / `hover:bg-[#2d2d2d]` dark) for icon-only buttons and `TEXT_BTN_CLASS` (`h-[26px] px-1.5 text-[10px]`, same hover palette) for HTML/PDF text chips; the cluster is wrapped in a `flex-wrap justify-end` container preceded by a 1px vertical divider visible at the `sm:` breakpoint (hidden on mobile and in the floating variant). The metadata trigger (`ConversationMetadataPopover`) renders a flat icon-button matching `ICON_BTN_CLASS` (no more circular bordered chip), and the `ChatHeaderOverflowMenu` trigger renders horizontal three-dot SVG dots (`⋯` semantics) for consistency with the rest of the cluster. **Resume in CLI** is not a top-bar button — it lives only inside the metadata (`ⓘ`) popover (wide tier) and the overflow menu (medium/narrow desktop), so the header chrome stays focused on identity + status. The inline `ContextWindowIndicator` has been removed from the header — context-window state now lives next to the composer's Send button. The composer's stacked toolbar carries a new `ComposerMetaStrip` between the attach button and Send: a `cwd` chip (working directory shortened from the head) and a context-window fuel gauge (64×6 bar, green→amber→red at 60%/80%, tabular-nums percent). The strip is omitted in the legacy `compactModeSelector` single-row layout.
