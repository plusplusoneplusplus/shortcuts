...
│               ├── features/chat/       # Chat conversation UI: ChatDetail/ChatListPane, FollowUpInputArea + NewChatArea use a compact stacked layout. The input card holds the editor and a single mobile-responsive (`flex-wrap`) toolbar containing — in this order — the ModePillSelector (rectangular `rounded-md` segmented control: pills `px-2.5 py-[3px] text-[11.5px]`, active pill uses an inset border shadow `shadow-[inset_0_0_0_1px_#d0d0d0]` instead of an outer border, dot colors match `MODE_BORDER_COLORS`: ask=yellow, plan=blue, autopilot=green), the model picker chip (single source of truth for the active model — shows session model or override and an inline ✕ to clear; no separate override badge), uniform `ctool` buttons (slash, `@` mention-skill, attach — each `h-[26px] px-[7px] rounded-sm text-[11.5px]` matching the OpenDesign reference), and the QueueFollowUpButton (`h-[28px] pl-2.5 pr-2 text-[12px]` with a `sm:`-gated ⌘↵ shortcut hint that uses a vertical `border-l` separator instead of a boxed kbd). The `@` mention-skill button shares the slash button's handler — both open the skill picker. Compact density: outer `py-2`, stack `space-y-1`, RichTextInput `min-h-[28px] px-3 pt-2 pb-1 text-[13.5px]`, toolbar `pl-2.5 pr-2 py-1.5`. Legacy single-row layout is preserved when `compactModeSelector` is true (e.g., narrow side panels). SlashCommandMenu renders as a card popover with a header and `↵` indicator on the highlighted row.
