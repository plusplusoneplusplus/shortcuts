...
export class RepoTreeService {
    ...
    private async walkFiles(
        ...
        const walk = async (dir: string): Promise<void> => {
            ...
            if (!showIgnored) {
                // ripgrep excludes .git even under --hidden, and so does the
                // native walker; without this the two disagree on any machine
                // where rg is missing and this walk runs instead.
                filtered = filtered.filter(e => !(e.isDir && e.name === '.git'));
