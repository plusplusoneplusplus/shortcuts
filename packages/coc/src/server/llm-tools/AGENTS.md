...
| `ask-user-tool.ts` | `ask_user` | Poses structured questions to the user (select, multi-select, yes/no, confirm, text) and blocks until the user responds. Returns a Promise resolved by `answerQuestion()`/`skipQuestion()`. The executor persists the current pending payload on `AIProcess.pendingAskUser`, emits an SSE event so the SPA renders the interactive widget, and clears the payload on answer, skip, or executor cleanup. |
