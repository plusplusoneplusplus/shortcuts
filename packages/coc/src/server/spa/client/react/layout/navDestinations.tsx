/**
 * navDestinations — the admin destinations the mobile shell offers, and their
 * icons.
 *
 * Shared by `BottomNav` (which owns the row off the repos tab) and
 * `MobileScopeBar` (which folds the same list into its `⋯` sheet on the repos
 * tab), so the two can never offer different destinations. Kept in its own
 * module rather than exported from `BottomNav` so neither component has to
 * import the other.
 */
...
/**
 * The admin destinations this nav offers. Exported so `MobileScopeBar` — which
 * replaces this nav on the repos tab — can fold the same list into its `⋯`
 * sheet rather than keeping a second copy that drifts.
 */
export function getNavItems(serversEnabled: boolean): NavItem[] {
