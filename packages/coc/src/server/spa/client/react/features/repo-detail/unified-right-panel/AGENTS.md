...
| `UnifiedPanelTabStrip.tsx` + `UnifiedPanelTabContextMenu.tsx` + `unifiedPanelTabMenuModel.ts` | Presentational strip and accessible VS Code-style tab menu. A dedicated visual divider in the tab row separates workspace tools from chat resources; the workspace below remains continuous. The pure model owns per-kind action availability, path resolution, and visible-order bulk target selection. |
