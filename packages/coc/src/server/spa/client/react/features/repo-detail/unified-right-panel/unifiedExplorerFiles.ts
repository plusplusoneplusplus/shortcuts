 ...
 * `{ path, name, line? }` into the descriptor the panel files.
 *
 ...
 *  - Trusted absolute paths remain byte-identical so `PreviewPane` can apply its
 *    own trusted-path guard and omit the write route.
 *
...
export function explorerFileTabInput(
    ...
    return {
        ...
        ...(file.symbolCandidate ? { symbolCandidate: true } : {}),
    };
}
