...
const EMPTY_INSTRUCTIONS: Record<InstructionMode, string | null> = {
    base: null,
    ask: null,
    plan: null,
    autopilot: null,
...
export function RepoSettingsTab({ workspaceId, repo }: RepoSettingsTabProps) {
    ...
    // ── Instructions state ───────────────────────────────────────────────────
    const [instrContents, setInstrContents] = useState<Record<InstructionMode, string | null>>(EMPTY_INSTRUCTIONS);
    ...
    const fetchInstructions = useCallback(async () => {
        ...
        try {
            const data = await getSpaCocClient().workspaces.getInstructions(workspaceId) as Partial<Record<InstructionMode, string | null>> | null | undefined;
            const normalized: Record<InstructionMode, string | null> = {
                base: data?.base ?? null,
                ask: data?.ask ?? null,
                plan: data?.plan ?? null,
                autopilot: data?.autopilot ?? null,
            };
            setInstrContents(normalized);
            setInstrDraft({
                base: normalized.base ?? '',
                ask: normalized.ask ?? '',
                plan: normalized.plan ?? '',
                autopilot: normalized.autopilot ?? '',
            });
        } catch {
            // ignore
        } finally {
