...
export interface RemoteShellHeaderProps {
    repo?: RepoData;
    repos: RepoData[];
    /** Suppress the workspace identity chip when the ScopeSlideSwitcher owns identity. */
...
export function RemoteShellHeader({ repo, repos, hideIdentity }: RemoteShellHeaderProps) {
    return (
        <div className="hidden md:flex items-center gap-1.5 min-w-0 flex-1" data-testid="remote-shell-header">
            <RemoteScopeCluster repo={repo} repos={repos} hideIdentity={hideIdentity} />
            {repo && (
                <>
