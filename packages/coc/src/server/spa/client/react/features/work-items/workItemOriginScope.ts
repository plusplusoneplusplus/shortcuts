...
export interface WorkItemOriginScopeInput {
    workspaceId: string;
    remoteUrl?: string | null;
}

export interface WorkItemOriginScope {
    originId: string;
    workspaceId: string;
}
...
export function resolveWorkItemOriginId(input: WorkItemOriginScopeInput): string {
    ...
    if (!remoteUrl) {
        return `local_${encodeOriginSegment(workspaceId)}`;
    }

    const normalizedRemoteUrl = normalizeRemoteUrlForOrigin(remoteUrl);
    const parts = normalizedRemoteUrl.split('/').filter(Boolean);
    const host = parts[0];

    if (host === 'github.com' && parts[1] && parts[2]) {
        return `gh_${encodeOriginSegment(parts[1])}_${encodeOriginSegment(parts[2])}`;
    }

    if (host === 'dev.azure.com' && parts[1] && parts[2]) {
        return `ado_${encodeOriginSegment(parts[1])}_${encodeOriginSegment(parts[2])}`;
    }

    const hashSource = normalizedRemoteUrl || remoteUrl.toLowerCase().replace(/^(https?:\/\/)([^@]+@)/, '$1').replace(/\/$/, '').replace(/\.git$/, '').replace(/\/$/, '');
    return `git_${sha256Hex(hashSource).slice(0, 16)}`;
}

export function resolveWorkItemOriginScope(input: WorkItemOriginScopeInput): WorkItemOriginScope {
    return {
        originId: resolveWorkItemOriginId(input),
...
export function resolveRepoWorkItemOriginScope(repo: RepoData): WorkItemOriginScope {
    ...
    return resolveWorkItemOriginScope({ workspaceId, remoteUrl });
}
