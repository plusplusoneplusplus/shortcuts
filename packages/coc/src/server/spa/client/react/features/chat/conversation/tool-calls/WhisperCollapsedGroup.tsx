...
// ---------------------------------------------------------------------------
// SkillPopoverRow — a clickable skill name that opens panel-centered detail
// ---------------------------------------------------------------------------

interface SkillPopoverRowProps {
    name: string;
    onSelect: (name: string, row: HTMLElement | null) => void;
}

function SkillPopoverRow({ name, onSelect }: SkillPopoverRowProps) {
    const rowRef = useRef<HTMLDivElement | null>(null);
    return (
        <div
            ref={rowRef}
            className="flex items-center gap-2 px-2.5 py-1 text-xs cursor-pointer hover:bg-[#e1effe] dark:hover:bg-[#1f2d42]"
            data-testid="skill-popover-row"
            role="button"
            tabIndex={0}
            aria-label={`Show detail for skill ${name}`}
            onClick={e => { e.stopPropagation(); onSelect(name, rowRef.current); }}
            onKeyDown={e => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    e.stopPropagation();
                    onSelect(name, rowRef.current);
                }
            }}
        >
            ...
            <span className="shrink-0 text-[10px] text-[#848484]" aria-hidden="true">▶</span>
        </div>
    );
}
...
interface SkillHoverPopoverProps {
    ...
    onMouseEnter: () => void;
    onMouseLeave: () => void;
    onSelectSkill: (name: string, row: HTMLElement | null) => void;
...
function SkillHoverPopover({
    ...
    useEffect(() => {
        ...
        const frame = requestAnimationFrame(() => {
            popoverRef.current?.querySelector<HTMLElement>('[data-testid="skill-popover-row"]')?.focus();
            onFirstRowFocused?.();
        });
        return () => cancelAnimationFrame(frame);
    }, [focusFirstRow, onFirstRowFocused, popoverRef]);
...
function SkillHoverSpan({ text, skillNames, workspaceId, testId }: SkillHoverSpanProps) {
    const { hovered, anchorRef, popoverRef, showPopover, hidePopover, dismissPopover } = useHoverPopover<HTMLSpanElement>();
    const skillDetailDialog = useWhisperSkillDetailDialog();
    const isSkillDetailOpen = skillDetailDialog?.isOpen ?? false;
    const [focusListOnOpen, setFocusListOnOpen] = useState(false);
    ...
    }, [dismissPopover, isSkillDetailOpen]);

    const showSkillPopover = useCallback(() => {
        if (isSkillDetailOpen || suppressPopover) return;
        showPopover();
    }, [isSkillDetailOpen, showPopover, suppressPopover]);

    const hideSkillPopover = useCallback(() => {
        hidePopover();
    }, [hidePopover]);

    const handleSelectSkill = useCallback((name: string, row: HTMLElement | null) => {
        setFocusListOnOpen(false);
        setSuppressPopover(true);
        dismissPopover();
        skillDetailDialog?.openSkillDetail({
            name,
            workspaceId,
            returnFocusTo: row,
            fallbackFocusTo: anchorRef.current,
        });
    }, [anchorRef, dismissPopover, skillDetailDialog, workspaceId]);

    const handleTriggerKeyDown = useCallback((event: React.KeyboardEvent<HTMLSpanElement>) => {
        if (event.key !== 'Enter' && event.key !== ' ') return;
        event.preventDefault();
        event.stopPropagation();
        setFocusListOnOpen(true);
        showSkillPopover();
    }, [showSkillPopover]);

    const handleFirstRowFocused = useCallback(() => {
        setFocusListOnOpen(false);
    }, []);
    ...
    return (
        <span
            ref={anchorRef}
            onMouseEnter={showSkillPopover}
            onMouseLeave={hideSkillPopover}
            onFocus={showSkillPopover}
            onClick={event => event.stopPropagation()}
            onKeyDown={handleTriggerKeyDown}
            className="underline decoration-dotted cursor-pointer focus:outline-none focus:ring-1 focus:ring-[#0078d4] rounded-sm"
            role="button"
            tabIndex={0}
            aria-haspopup="menu"
            data-testid={testId}
        >
            {text}
            {hovered && !isSkillDetailOpen && !suppressPopover && skillNames.length > 0 && (
                <SkillHoverPopover
                    ...
                    focusFirstRow={focusListOnOpen}
                    onFirstRowFocused={handleFirstRowFocused}
                />
            )}
        </span>
    );
