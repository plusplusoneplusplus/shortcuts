 ...
 *
 * The row stays a single row at every width: the tab strip scrolls horizontally
 * (scrollbar hidden) instead of squeezing, and on mobile the labelled action
 * buttons collapse into a `⋯` menu so the header never overflows a 375px
 * viewport.
 */
import { cn } from '../../ui';
import { useBreakpoint } from '../../hooks/ui/useBreakpoint';
import { useVirtualWorkspaceHeader } from './useVirtualWorkspaceHeader';
import { VirtualWorkspaceHeaderActionsMenu } from './VirtualWorkspaceHeaderActionsMenu';
...
export function VirtualWorkspaceInlineHeader({ config }: VirtualWorkspaceInlineHeaderProps) {
    ...
    return (
        <div
            className="flex items-center min-w-0 px-3 border-b border-[#e0e0e0] dark:border-[#3c3c3c] bg-[#f8f8f8] dark:bg-[#2d2d2d] flex-shrink-0"
            data-testid={`${prefix}-header`}
        >
            <span className="text-sm font-semibold text-[#333] dark:text-[#ccc] mr-2 flex-shrink-0">
                {config.icon} {config.label}
            </span>
            {/* Tab strip — scrolls horizontally rather than squeezing the row. */}
            <div
                className="flex items-center flex-1 min-w-0 overflow-x-auto scrollbar-hide"
                data-testid={`${prefix}-header-tabs`}
            >
                {visibleTabs.map(t => (
                    <button
                        key={t.key}
                        ...
                        title={t.shortcut}
                        className={cn(
                            'touch-target text-xs font-medium transition-colors relative whitespace-nowrap shrink-0 px-3 py-2',
                            activeTab === t.key
            ...
            />
            {/* Action buttons — collapsed behind `⋯` on mobile. */}
            <div className="flex items-center gap-2 flex-shrink-0">
                {isMobile ? (
                    <VirtualWorkspaceHeaderActionsMenu
                ...
                {statusMsg && (
                    <span
                        className="text-xs text-[#666] dark:text-[#999] ml-1 whitespace-nowrap truncate max-w-[35vw] md:max-w-none"
