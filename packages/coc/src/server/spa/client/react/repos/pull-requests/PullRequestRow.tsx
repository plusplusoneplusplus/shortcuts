...
export function PullRequestRow({ pr, onClick }: PullRequestRowProps) {
    ...
    return (
        <div
            className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer border-b border-gray-100 dark:border-gray-700"
            onClick={onClick}
            data-testid="pr-row"
        >
            <span
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium shrink-0 mt-0.5 ${badge.className}`}
                title={badge.label}
            >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    {pr.number != null && (
                        <span className="text-xs text-gray-400 shrink-0">#{pr.number}</span>
                    )}
