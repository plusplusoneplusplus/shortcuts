        ...
        <div className="flex flex-col gap-2 overflow-y-auto max-h-[400px]">
            {items.map((entry, i) => {
                ...
                return (
                    ...
                    >
                        {/* Header row: timestamp + mode badge */}
                        <div className="flex items-center justify-between mb-1">
                            <span className="text-[11px] text-[#848484]">🕐 {relativeTime(entry.timestamp)}</span>
                            {entry.mode && (
                        ...
                        {/* Body: prompt preview */}
                        <div className="text-xs text-[#1e1e1e] dark:text-[#cccccc] line-clamp-2 mb-1">
                            {bodyText}
                        </div>

                        {/* Footer: skill chips + model */}
                        {(skillChips || entry.model) && (
                            <div className="flex items-center justify-between mt-1">
                                <div className="flex flex-wrap gap-1">
                                    {skillChips?.map(s => (
                                        <span key={s} className="text-[10px] text-[#848484]">⚡ {s}</span>
                                    ))}
                                </div>
                                {entry.model && (
                                    <span className="text-[10px] text-[#848484] ml-auto">{entry.model}</span>
                                )}
                            </div>
                        )}
                    </button>
