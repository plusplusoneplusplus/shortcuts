 ...
 *
 * The user message is minimal: a one-line pointer to the ultra-ralph skill
 * (which carries the full machine contract, RALPH_NEXT/RALPH_COMPLETE grammar,
 * and iteration-header spec), an optional dynamic context block with the
 * progress-journal path and iteration counter, then the goal in a <goal>
 * block (goal last per the AC-01 constraint: skill pointer + dynamic context
 * first, <goal> last).
 */
 ...
 *
 * Structure: skill pointer → dynamic context (when available) → <goal> block.
 */
