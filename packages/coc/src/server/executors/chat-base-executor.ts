...
export interface ChatModeAIOptions {
    ...
    dispose?: () => void;
    /**
     * Dangerous-command guard wiring for this turn. Set only by the ask-mode
     * path — autopilot and Ralph are explicitly user-authorized and are not
     * gated. Omitted (or `{ enabled: false }`) leaves the turn exactly as it
     * was before the guard existed.
     */
    dangerousCommandGuard?: DangerousCommandGuardOptions;
...
export abstract class ChatBaseExecutor extends BaseExecutor {
    ...
    /** The `dangerousCommandGuard.enabled` admin flag, read per turn. */
    protected get dangerousCommandGuardEnabled(): boolean { return this.queueConfig.getDangerousCommandGuard().enabled; }
    ...
    /**
     * Build the dangerous-command guard block for a turn.
     *
     * The approval prompt rides the `ask_user` channel, so it resolves the
     * handles lazily: they are registered right after the turn's tool context
     * is built, and a Ralph grill terminal round clears them mid-turn.
     */
    protected buildDangerousCommandGuardWiring(
        processId: string,
        opts: {
            isInteractive: () => boolean;
            /**
             * Mode gate, ANDed with the admin flag. Ask mode only — a follow-up
             * reads it off the turn's current mode pill, which can differ from
             * the mode the chat was opened in.
             */
            enabled?: boolean;
        },
    ): DangerousCommandGuardOptions {
        return buildDangerousCommandGuardWiring({
            processId,
            enabled: this.dangerousCommandGuardEnabled && opts.enabled !== false,
    ...
    protected async buildStandardModeOptions(
        ...
        this.setAskUserHandles(processId, {
            askApproval: ctx.askUser!.askApproval,
        ...
        return {
            ...
            dispose: ctx.dispose,
            // Ask mode only. Autopilot's shell calls are already
            // user-authorized by the mode itself, so gating them would prompt
            // for something the user just asked for.
            ...(isAsk
                ? {
                    dangerousCommandGuard: this.buildDangerousCommandGuardWiring(processId, {
                        // A dashboard chat's first turn is opened by a person
                        // who is watching it — the same assumption the ask-user
                        // wiring above makes by leaving `isInteractive` unset.
                        isInteractive: () => true,
                    }),
                }
                : {}),
    ...
    async execute(task: QueuedTask, prompt: string): Promise<ChatModeExecutionResult> {
        ...
        let { agentMode, systemMessage, tools, effectivePrompt, excludedTools, dispose: modeDispose, ralphGrillPlanning, dangerousCommandGuard } = await this.buildModeOptions(task, prompt, workingDirectory);
