...
export function registerAdminRoutes(routes: Route[], options: AdminRouteOptions): void {
    ...
    routes.push({
        ...
        handler: async (req, res) => {
            ...
            const editableKeys = ['model', 'parallel', 'timeout', 'output', 'showReportIntent', 'toolCompactness', 'taskCardDensity', 'groupSingleLineMessages', 'chat.followUpSuggestions.enabled', 'chat.followUpSuggestions.count', 'serve.serverName', 'terminal.enabled', 'notes.enabled', 'myWork.enabled', 'myLife.enabled', 'clientPool.enabled', 'clientPool.size'];
