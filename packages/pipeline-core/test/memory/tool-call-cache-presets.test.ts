...
describe('EXPLORE_FILTER', () => {
    it('matches grep', () => {
        expect(EXPLORE_FILTER('grep', { pattern: 'foo' })).toBe(true);
    });

    it('matches glob', () => {
        expect(EXPLORE_FILTER('glob', { pattern: '**/*.ts' })).toBe(true);
    });

    it('matches view', () => {
        expect(EXPLORE_FILTER('view', { path: '/src/index.ts' })).toBe(true);
    });

    it('matches read_file', () => {
        expect(EXPLORE_FILTER('read_file', {})).toBe(true);
    });

    it('matches list_directory', () => {
        expect(EXPLORE_FILTER('list_directory', {})).toBe(true);
    });
    ...
    it('rejects task with agent_type=general-purpose', () => {
        expect(EXPLORE_FILTER('task', { agent_type: 'general-purpose' })).toBe(false);
    });

    it('rejects task with no agent_type', () => {
        expect(EXPLORE_FILTER('task', {})).toBe(false);
    });
