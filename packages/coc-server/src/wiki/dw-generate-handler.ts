// @ts-nocheck — Verbatim copy from deep-wiki; dynamic imports reference unavailable deep-wiki modules
/**
 * Generate Handler
 *
...
async function runGeneration(
    ...
    const {
        ...
        runPhase4Writing,
        runPhase5Website,
    // @ts-expect-error deep-wiki module not available in CoC
...
async function handleComponentRegenerate(
    ...
    try {
        ...
        const outputDir = context.wikiDir;
        analysis = getCachedAnalysis(componentId, outputDir);
    } catch {
        // Cache module not available (e.g., running in CoC without deep-wiki)
    }
