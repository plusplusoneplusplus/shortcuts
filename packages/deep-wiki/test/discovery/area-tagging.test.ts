 ...
 * Area Tagging Tests
 *
 * Tests for mergeSubGraphs area tagging in large-repo-handler:
 * - Modules tagged with area slug
 * - AreaInfo[] populated from TopLevelArea[]
 * - Backward compat: no areas for empty scan result
 * - Module-to-area assignment tracking
 */

import { describe, it, expect } from 'vitest';
import { mergeSubGraphs } from '../../src/discovery/large-repo-handler';
import type { ModuleGraph, StructuralScanResult, AreaInfo } from '../../src/types';
...
describe('mergeSubGraphs — area tagging', () => {
    it('should tag modules with area slug from TopLevelArea path', () => {
        ...
        expect(authModule.area).toBe(normalizeModuleId('packages/core'));
        expect(dbModule.area).toBe(normalizeModuleId('packages/core'));
        expect(routesModule.area).toBe(normalizeModuleId('packages/api'));
    });

    it('should populate graph.areas from TopLevelArea[]', () => {
        const graph1 = createMinimalGraph([
        ...
        const coreArea = result.areas!.find(a => a.name === 'Core')!;
    ...
    it('should correctly assign modules to their respective areas', () => {
        const graph1 = createMinimalGraph([
        ...
        const area1 = result.areas!.find(a => a.id === 'area1')!;
        expect(area1.modules).toEqual(['mod-a', 'mod-b']);

        const area2 = result.areas!.find(a => a.id === 'area2')!;
        expect(area2.modules).toEqual(['mod-c']);
    });

    it('should not produce areas when scan result has empty areas array', () => {
        const graph1 = createMinimalGraph([
    ...
    it('should handle area with no modules (empty sub-graph)', () => {
        const graph1 = createMinimalGraph([
            { id: 'core-auth', name: 'Auth', path: 'core/auth/' },
        ]);
        ...
        const scanResult: StructuralScanResult = {
            ...
            areas: [
                ...
                { name: 'Empty', path: 'empty', description: 'Empty area' },
            ],
            projectInfo: {},
        };
        ...
        expect(result.areas).toHaveLength(2);
        const emptyArea = result.areas!.find(a => a.id === 'empty')!;
        expect(emptyArea.modules).toEqual([]);
    });

    it('should normalize area paths with special characters into kebab-case slugs', () => {
        const graph1 = createMinimalGraph([
        ...
        expect(area.id).toBe('my-packages-core-v2');
        expect(result.modules[0].area).toBe('my-packages-core-v2');
    });

    it('should deduplicate modules across areas (first wins)', () => {
        const graph1 = createMinimalGraph([
        ...
        // Should be tagged with first area
        expect(result.modules[0].area).toBe('area1');

        // Should appear in first area's module list
        const area1 = result.areas!.find(a => a.id === 'area1')!;
