...
describe('buildProbePrompt', () => {
    ...
    it('should include the repo path', () => {
        const prompt = buildProbePrompt('/path/to/repo', theme);
        expect(prompt).toContain('/path/to/repo');
    });

    it('should include the theme name', () => {
        const prompt = buildProbePrompt('/repo', theme);
        expect(prompt).toContain('authentication');
    });

    it('should include the theme description', () => {
        const prompt = buildProbePrompt('/repo', theme);
    ...
    it('should include the JSON schema', () => {
        ...
        expect(prompt).toContain('"discoveredThemes"');
    ...
    it('should handle theme with no hints', () => {
        const themeNoHints: ThemeSeed = {
            theme: 'api-gateway',
            description: 'API gateway implementation',
            hints: [],
        };
        const prompt = buildProbePrompt('/repo', themeNoHints);
        expect(prompt).toContain('api-gateway');
    });

    describe('focus domain', () => {
        it('should include focus section when focus is provided', () => {
            const prompt = buildProbePrompt('/repo', theme, 'src/');
    ...
    it('should handle special characters in theme names', () => {
        const specialTheme: ThemeSeed = {
            theme: 'api-gateway-v2',
            description: 'API gateway v2 with special chars: <>&"',
            hints: ['api', 'gateway'],
        };
        const prompt = buildProbePrompt('/repo', specialTheme);
