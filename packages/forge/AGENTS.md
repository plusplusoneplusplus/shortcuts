...
| **process-store** | `src/process-store.ts`, `src/file-process-store.ts`, `src/sqlite-process-store.ts` | Abstract `ProcessStore` + `FileProcessStore` (per-repo directory layout, 500-process cap) + `SqliteProcessStore` (schema version 5, FTS5 `conversation_search` index on `conversation_turns.content` with sync triggers). `lastEventAt` is set on `addProcess` (= `startTime`) and updated to current time on `appendConversationTurn`. |
