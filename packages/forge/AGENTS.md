...
| **queue** | `src/queue/` | `TaskQueueManager` (priority queue, indefinite/timed queue + autopilot pauses) + `QueueExecutor` (concurrency control, events) |
...
| **copilot-sdk** | `src/copilot-sdk-wrapper/` | `CopilotSDKService` (singleton, session-per-request pattern with error recovery, `createClient`/`sendMessage`/`abortSession`/`transform`), `ModelRegistry`, model metadata cache and reasoning-effort resolver, MCP config loader, SDK availability detection, trusted folders. `StreamingSession` gates settlement on SDK background tasks (`session.idle` with `backgroundTasks` field, `background_tasks_changed` event) — defers `settleWithResult()` until all agents/shells drain to zero. |
| **process-store** | `src/process-store.ts`, `src/file-process-store.ts`, `src/sqlite-process-store.ts` | Abstract `ProcessStore` + `FileProcessStore` (per-repo directory layout, 500-process cap) + `SqliteProcessStore` (schema version 6, FTS5 `conversation_search` index on `conversation_turns.content` with sync triggers). `lastEventAt` is set on `addProcess` (= `startTime`) and updated to current time on `appendConversationTurn`. Pin/archive state stored on `processes` table (`pinned_at TEXT`, `archived INTEGER`); process-runtime state such as `pendingMessages` and `pendingAskUser` is folded into persisted process metadata; convenience methods include `pinProcess`, `unpinProcess`, `archiveProcess`, `unarchiveProcess`, `archiveProcesses`, `unarchiveProcesses`, `getPinnedProcesses`. Prompt-autocomplete helpers `getBestPromptCompletion` and `getPromptAutocompleteContext` provide deterministic suffix lookup plus bounded user-history context for AI-generated ghost text. |
...
| **editor** | `src/editor/` | Comment anchors, markdown parsing/rendering, opt-in HTML embed title parsing, Google Maps embed URL allowlisting, file state, message transport |
...
| **skills** | `src/skills/` | Skill management: source detector, skill scanner, skill installer, general bundled skills provider (`getBundledSkillsRegistry`, `parseBundledSkillVersion`, `parseSkillVersionFromFile`), My Work bundled skills provider (`autoInstallMyWorkSkills`), skill updater (`autoUpdateBundledSkills` — version-aware auto-update of globally-installed skills), and skill resolver (`skill-resolver.ts` — resolves and loads skill prompts from `.github/skills/`). Version is read from each skill's `SKILL.md` frontmatter `metadata.version` at runtime (no hardcoded versions in registries). |

Entry point: `src/index.ts` — re-exports all public API from the modules above.

## Memory System

Bounded, file-backed memory lets AI chat sessions learn from past interactions. Direct bounded memory actions mutate `MEMORY.md`; chat-time capture mode upserts durable candidate rows instead of rewriting bounded memory. Capture assigns candidate scores from caller-controlled context (`explicitMemoryIntent`, `writeFrequency`) rather than model-provided parameters. Candidate ranking is deterministic and explainable from stored metadata; explicit memory intent satisfies the relevance signal for selection while keeping the minimal diversity sanity check. The frozen bounded-memory snapshot is injected into subsequent prompts.
