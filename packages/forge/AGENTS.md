...
| **process-store** | `src/process-store.ts`, `src/file-process-store.ts`, `src/sqlite-process-store.ts` | Abstract `ProcessStore` + `FileProcessStore` (per-repo directory layout, 500-process cap) + `SqliteProcessStore` (schema version 4, `last_event_at` column for conversation-event tracking). `lastEventAt` is set on `addProcess` (= `startTime`) and updated to current time on `appendConversationTurn`. |
