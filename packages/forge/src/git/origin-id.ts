 ...
 *
 * Pure and synchronous — no Node.js dependencies (safe for browser bundles).
 * The server and the SPA both derive origin IDs from this module so that
 * persisted origin-scoped keys agree byte-for-byte.
 *
 * Canonical origin IDs are:
 *   - GitHub: `gh_<owner>_<repo>`
 *   - Azure DevOps: `ado_<org>_<project>` (organization/project scoped)
 ...
 *
 * Shared by the historical `computeRemoteHash` in `remote.ts` and by the
 * canonical-origin hash fallback below; the two hash paths keep their separate
 * semantics but must not drift on the normalisation itself.
 *
 * @param url Raw remote URL (e.g. `https://github.com/owner/repo.git`).
 */
export function normalizeRemoteUrlForHash(url: string): string {
...
function computeCanonicalRemoteHash(remoteUrl: string): string {
    const normalized = normalizeRemoteUrlForOrigin(remoteUrl) || normalizeRemoteUrlForHash(remoteUrl);
 ...
 *
 * Throws when there is no remote URL and no workspace ID: a local origin has no
 * other identity to fall back on, and silently returning `local_` would collapse
 * every unidentified workspace into one shared scope.
 */
