/**
 * Regression coverage that forge's AI timeout defaults stay in lockstep with
 * the provider SDK package. forge carries its own copies because
 * `@plusplusoneplusplus/coc-agent-sdk` cannot import from forge (forge depends
 * on it), so nothing but a test stops the two from drifting apart.
 */
...
describe('forge AI timeout defaults', () => {
