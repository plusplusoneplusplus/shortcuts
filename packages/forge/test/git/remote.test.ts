/**
 * The pure-string half of `remote.ts`: normalisation, hashing and canonical
 * origin ids.
 *
 * Reading a remote off a repository is native now and is covered against real
 * temporary repositories in `remote-native.test.ts` — the tests that used to
 * live here drove a mocked `execGitAsync` call sequence that no longer exists.
 */
