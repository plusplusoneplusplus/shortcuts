...
describe('sqlite-schema', () => {
    ...
    describe('V24 → V25 migration (display_only on conversation_turns)', () => {
        it('fresh DB includes display_only column on conversation_turns', () => {
            initializeDatabase(db);

            const cols = db.prepare("PRAGMA table_info(conversation_turns)").all() as Array<{ name: string }>;
            expect(cols.map(c => c.name)).toContain('display_only');
        });

        it('adds display_only to an existing V24 database without data loss', () => {
            // Simulate a V24 conversation_turns table that lacks display_only
            db.exec(`
                CREATE TABLE processes (
                    id                    TEXT PRIMARY KEY,
                    workspace_id          TEXT NOT NULL,
            ...
            db.prepare(`
                INSERT INTO processes (id, workspace_id, status, start_time)
                VALUES ('p-v24', 'ws1', 'completed', '2026-01-01T00:00:00.000Z')
            `).run();
            db.prepare(`
                INSERT INTO conversation_turns (process_id, turn_index, role, content, timestamp)
                VALUES ('p-v24', 0, 'assistant', 'hello', '2026-01-01T00:00:01.000Z')
            `).run();
            ...
            // Existing turn preserved with the column defaulting to 0 (not display-only).
            const row = db.prepare(`
                SELECT content, display_only FROM conversation_turns WHERE process_id = 'p-v24'
            `).get() as any;
            expect(row.content).toBe('hello');
            expect(row.display_only).toBe(0);

            // New column is writable.
            db.prepare('UPDATE conversation_turns SET display_only = 1 WHERE process_id = ? AND turn_index = ?')
                .run('p-v24', 0);
            const updated = db.prepare(`
    ...
    describe('V27 → V28 migration (repo_group_context on conversation_turns)', () => {
        it('fresh DB includes repo_group_context column on conversation_turns', () => {
            initializeDatabase(db);

            const cols = db.prepare("PRAGMA table_info(conversation_turns)").all() as Array<{ name: string }>;
            expect(cols.map(c => c.name)).toContain('repo_group_context');
        });

        it('adds repo_group_context to an existing V27 database without data loss', () => {
            db.exec(`
                CREATE TABLE processes (
                    id                    TEXT PRIMARY KEY,
                    workspace_id          TEXT NOT NULL,
            ...
            db.prepare(`
                INSERT INTO processes (id, workspace_id, status, start_time)
                VALUES ('p-v27', 'ws1', 'completed', '2026-01-01T00:00:00.000Z')
            `).run();
            db.prepare(`
                INSERT INTO conversation_turns (process_id, turn_index, role, content, timestamp)
                VALUES ('p-v27', 0, 'user', 'hello', '2026-01-01T00:00:01.000Z')
            `).run();
            ...
            // Existing turn preserved with null repo_group_context.
            const row = db.prepare(`
                SELECT content, repo_group_context FROM conversation_turns WHERE process_id = 'p-v27'
            `).get() as any;
            expect(row.content).toBe('hello');
            expect(row.repo_group_context).toBeNull();

            // New column is writable.
            db.prepare('UPDATE conversation_turns SET repo_group_context = ? WHERE process_id = ? AND turn_index = ?')
                .run('<repo_group_context>x</repo_group_context>', 'p-v27', 0);
            const updated = db.prepare(`
    ...
    describe('V34 -> V35 migration (per-turn provider attribution)', () => {
        it('adds a nullable provider column without backfilling existing turns', () => {
            db.exec(`
                CREATE TABLE processes (
                    id                    TEXT PRIMARY KEY,
                    workspace_id          TEXT NOT NULL,
            ...
            `);
            db.prepare(`
                INSERT INTO conversation_turns (process_id, turn_index, role, content, timestamp, model)
                VALUES (?, ?, ?, ?, ?, ?)
            `).run('p-v34', 0, 'user', 'hello', '2026-01-01T00:00:00.000Z', 'gpt-5.4');
            ...
            expect(db.prepare(`
                SELECT role, content, model, provider
                FROM conversation_turns WHERE process_id = ?
            `).get('p-v34')).toEqual({
                role: 'user',
                content: 'hello',
                model: 'gpt-5.4',
