...
- **Network and credential operations always shell out** — `push`, `pull`, `fetch`, `clone` — so credential helpers, SSH agents and 2FA keep working exactly as they do for a human at a terminal. Never through a Rust git library. Shelling out from *Rust* is still the point: `POST /api/git/clone` and the notes `resetFromOrigin` both run `git clone` through `execGitAsync` with `timeout: 0`, since a clone is bounded by repository size and network speed and no wall-clock cap is right for both a 2 MB repo and a 2 GB one.
- **No git child process is started from Node outside the WSL dispatch.** `grep -rnE "execFile|execSync|childProcess.exec" packages/coc/src packages/forge/src` (minus `spa/client`) leaves five hits and none of them is git on the native host: `forge/src/git/exec.ts` and `branch-service.ts` run `wsl.exe`, `coc/src/server/git/repo-utils.ts` and `routes/api-fs-routes.ts` run `wsl.exe`, and the GitHub work-item provider runs `gh api`. When you add a git call, reach for `execGitAsync` — adding a sixth is a regression, not a new case.
