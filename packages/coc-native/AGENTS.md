...
- **The blob is read, not `git show`.** Every command that crosses this boundary loses exactly one trailing newline. For a diff that is invisible; for a file's bytes it is not, so `file_content_at_commit` walks the tree and returns the blob verbatim. This is the one place in the capability where a command was replaced rather than ported *because* of the trailing-newline rule.
