...
- **The git-spawn inventory takes three greps, and it has to be run over `packages/*/src` — not over `coc` and `forge`.** Grep for the *command*, not for the API, because a runner's own file names neither:
  ...
  In `packages/coc/src` and `packages/forge/src` all three are clean: the only hit is `work-items/work-item-pr-submission-command.ts`, whose nine commands go through an injected runner whose default sends `git` to `execGitAsync` and only `gh pr create` to `execFile`. Three `wsl.exe` children run git and are the exempted dispatch: `forge/src/git/exec.ts` (`runGitViaWsl`), `forge/src/git/branch-service.ts` (its one shell pipeline), and `coc/src/server/git/repo-utils.ts` (`rev-parse --show-toplevel`, and the only synchronous one left in those two packages). The other `wsl.exe` calls — `coc/src/server/routes/api-fs-routes.ts` reading `$HOME`, `forge/src/utils/workspace-execution.ts` listing distros — are not git.
  
  **Two packages outside those two still spawn git, and the narrow scope is why nobody saw them:** `coc-agent-sdk/src/codex-sdk-service.ts` (`runGitDiffNoIndex` — a `git diff --no-index` of two temp files, one child per changed file; its three repository reads are native) and `deep-wiki/src/cache/git-utils.ts` plus `deep-wiki/src/utils/git-init.ts`, which run git through a **shell string** and include an `execSync('git init')` — the only synchronous git spawn left in the repository. Scripts are deliberately exempt: `coc-native/scripts/bench-git.mjs` forks children on purpose, and `coc/scripts/prebuild.mjs` and `scripts/skillopt/rollout.ts` are build tooling, not a request path.
  
  **An injected runner is how the last git spawn hid for five slices:** an `execFile` grep finds the runner and not its callers, so `work-item-execution-shared.ts` read as "runs `gh`", which is what it does for one call in nine. **A package the grep never covered is how the next ones hid:** the inventory was written as two paths, so five spawns in `coc-agent-sdk` and six in `deep-wiki` were never in scope to be found. When you add a git call, reach for `execGitAsync`.
