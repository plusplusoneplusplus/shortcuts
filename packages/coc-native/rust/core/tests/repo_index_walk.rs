...
#[test]
fn the_git_directory_is_skipped_even_when_ignored_files_are_included() {
    let dir = repo();
    write(dir.path(), ".gitignore", "build/\n");
    write(dir.path(), ".git/config", "");
    write(dir.path(), ".git/objects/ab/1f3c", "");
    write(dir.path(), ".git/logs/HEAD", "");
    write(dir.path(), "build/out.js", "");
    write(dir.path(), "a.ts", "");

    let listed = files(dir.path(), true);
    // `include_ignored` still surfaces gitignored files...
    assert!(listed.contains(&"build/out.js".to_string()));
    // ...but never anything under `.git`.
    assert!(
        listed.iter().all(|f| !f.starts_with(".git/")),
        "expected no .git entries, got {listed:?}"
    );
}
