...
use coc_native_core::score::{score_units, Query};
...
fn score(query: &str, path: &str) -> u32 {
    ...
    if path.is_ascii() {
        ...
        let target: Vec<u16> =
            path.encode_utf16().map(coc_native_core::score::lower_unit).collect();
...
fn matched_indices(query: &str, path: &str) -> Vec<u32> {
    let q = Query::new(query);
    let mut indices = Vec::new();
    let target: Vec<u16> = path.encode_utf16().map(coc_native_core::score::lower_unit).collect();
    score_units(&q.wide, &target, &mut indices);
    indices
}
