 ...
 * explanation when that guard vetoes a close. This module recognises the known,
 * same-origin PDF URL shapes and asks the host to confirm before overriding the
 * guard.
 *
 * `isSameOriginPdfChildUrl` is also part of the pop-out allow-list (see
 * `popout-chrome.ts`), and `wirePdfChildWindowClose` is applied by
 * `popout-window-host.ts` to the page view of a popped-out PDF.
 *
