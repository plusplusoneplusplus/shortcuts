...
**Feature flag:** `loops.enabled` in `~/.coc/config.yaml` (default `false`). When disabled, infrastructure is not constructed, REST routes are not registered, LLM tools (`scheduleWakeup`, `loop`) are filtered out, the bundled `/cron` skill is not auto-installed, and dashboard UI elements (badge, panel, slash-command) are hidden.
...
### `loop` (skill-gated — requires `/cron` skill)
...
- **`LoopManagementPanel`** — Panel listing all loops with pause/resume/cancel actions. Each active loop shows its next scheduled tick time (relative, e.g. "Next: in 24h") derived from `nextTickAt`, alongside tick count and last-tick time.
...
- **`/cron` slash command** — Auto-installs the `/cron` skill and activates loop tools for the session.
