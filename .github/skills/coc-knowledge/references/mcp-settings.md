...
When `mcpOauth.autoRefresh.enabled` is true, a background loop runs every 5 minutes against `~/.copilot/mcp-oauth-config/`: it dedups duplicate entries the SDK accumulates per `serverUrl` and refreshes AAD tokens within 10 minutes of expiry. Cached scopes are passed through `sanitizeRequestScope` first, which drops `<resource>/.default` when other scopes for the same resource exist (AAD rejects that combination with HTTP 400), and `offline_access` is then appended so refresh tokens keep rolling. Only `invalid_grant`, `interaction_required`, and `consent_required` delete the cache entry; every other 4xx (including `invalid_request`, `invalid_scope`) is treated as transient so request-shape bugs don't force a re-auth. Response bodies are logged (redacted, ≤400 chars) on failure to make AADSTS errors diagnosable. Interval/window are hardcoded. Disabled by default; restart required to take effect. Toggled in the admin Workspace Features card under "MCP OAuth auto-refresh" (only visible when the parent "MCP OAuth" toggle is on). Implementation in `packages/coc/src/server/mcp-oauth/mcp-oauth-refresher.ts`.
