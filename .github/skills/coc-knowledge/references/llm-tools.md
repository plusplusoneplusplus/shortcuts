...
| `canvas-tools.ts` | `create_canvas`, `update_canvas`, `read_canvas` | Chat canvas side-panel artifacts (type `markdown` for documents — Mermaid blocks render as diagrams — or `code` with a normalized `language` for a single code file). Gated by the `canvas.enabled` config flag (`buildCanvasToolsAddon` reads it from `<dataDir>/config.yaml`, with an injectable override for tests). Persists via `CanvasStore` (`~/.coc/repos/<wsId>/canvases/`), links the canvas to the creating process, applies revision-checked exact-match edits, and emits `canvas-updated` SSE events on the process channel. |
