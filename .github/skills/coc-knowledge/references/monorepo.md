...
TS glue (`npm run build`) needs no Rust; only `build:native` does, driving `@napi-rs/cli` to compile the addon and regenerate `src/native-bindings.ts` — the `#[napi]` type surface, committed so `tsc` never needs cargo, aliased by capability modules, and CI-gated by a regenerate-and-diff step. Binaries resolve from a locally built `coc-native.<triple>.node`, then `prebuilt/<triple>/`, and are ABI-stable across Node and Electron. `ensure:native` is the stale check that keeps a machine's binary current: it rebuilds only when the `.node` is missing or older than a file under `rust/`, and the serve loops (`coc-serve-loop.sh`/`.ps1`) run it before `coc:link` because nothing else in the local build path compiles the addon. A failed build or a missing cargo keeps the daemon on its existing binary; it only fails the build when there is no binary at all. `ensure:native` is the stale check that keeps a machine's binary current: it rebuilds only when the `.node` is missing or older than a file under `rust/`, and the serve loops (`coc-serve-loop.sh`/`.ps1`) run it before `coc:link` because nothing else in the local build path compiles the addon. A failed build or a missing cargo keeps the daemon on its existing binary; it only fails the build when there is no binary at all.
