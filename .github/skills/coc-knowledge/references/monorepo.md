...
**CoC build order:** `packages/coc-agent-sdk` builds before `coc-workflow`; `coc-workflow` builds before `forge` and `coc`; and `coc` depends on compiled `coc-agent-sdk`, `coc-workflow`, `coc-memory`, `forge`, `coc-client`, and `coc-connector` output. The root `build:packages` script builds `coc-agent-sdk`, `coc-workflow`, `forge`, `coc-client`, `coc-memory`, and `coc-connector` before `coc`. Direct `packages/forge` builds run `scripts/prebuild.mjs` to build `coc-agent-sdk` and `coc-workflow` before `tsc`; direct `packages/coc` builds run `scripts/prebuild.mjs` to build `coc-agent-sdk`, `coc-workflow`, `coc-memory`, `forge`, `coc-client`, and `coc-connector`, then clean `dist` before `tsc` emits package artifacts.
...
- **Desktop server port:** coc-desktop attaches to a healthy CoC server on the preferred port (4000 by default). Otherwise it starts its embedded server on that same port when available, falling back to a free ephemeral port only when another process already occupies it.
