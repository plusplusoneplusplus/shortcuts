...
| `processes/` | in-memory store, output-file-manager, stale-task-detector, pin/archive, seen-state, turn-actions, history, resume, and process-scoped mutation admission. Follow-up delivery re-reads process/task state inside the keyed admission section before any steer, buffer, enqueue, or transcript write, so a cross-provider request that loses the idle race fails with `PROVIDER_SWITCH_REQUIRES_IDLE` |
