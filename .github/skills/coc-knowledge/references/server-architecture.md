...
| `workspaces/` | global-workspace, my-work, my-life, repo-group, workspace-summary. In `repo-group-workspace.ts` a group is a virtual workspace (`group-<slug>` ID, root `~/.coc/repos/<groupId>/`) whose `group.json` holds `{ name, members: [workspaceId...] }` naming registered non-virtual repo workspaces only; names/paths resolve from the registry at read time, and removed or missing-path members come back stale. `repo-group-handler.ts` serves `/api/repo-groups` CRUD, wired in `routes/index.ts` with a `getWsServer` topology broadcaster and an `onGroupRegistered` hook registering the group with the queue bridge and schedule manager. `resolveRepoGroupChatContext` (`repo-group-chat-context.ts`) returns a `<repo_group_context>` block of each live member's name + absolute path plus the matching paths for `SendMessageOptions.additionalDirectories`. `ChatBaseExecutor.execute` and `FollowUpExecutor.executeFollowUp` pass `additionalDirectories` on every turn (a permission option, not conversation state). The block itself is session state: the first turn appends it (never to the persisted message text) and follow-ups re-append only when `shouldInjectRepoGroupContext` says the session cannot already have it — no resumable `sdkSessionId` (history rebuild), no earlier turn carrying it, a changed member listing, or a `/compact` since the last injection (detected from the display-only compaction result turn and `metadata.compaction.completedAt`; provider-side background compaction is not surfaced to the server and cannot be detected). Turns that carried the block record it verbatim via `persistRepoGroupContextOnUserTurn` (`ProcessStore.updateTurnRepoGroupContext`, `conversation_turns.repo_group_context`, schema V28) so the chat can disclose it behind a collapsed "Repo group context" toggle; that field is also the durable marker the injection decision reads back, so skipped turns record nothing |
...
| `notes/` | read/write/comments/AI/file-preview/image/edits handlers, `git/` sub-module, workspace-scoped multi-root resolution, and `notes-search-service.ts`. The roots API combines the managed root, user-configured Notes roots, and canonical task directories discovered from repo-scoped tasks, `.vscode/tasks`, and task `folderPaths`; task identities are opaque read-time-derived values, not Notes preferences. `notes-path-safety.ts` enforces lexical and canonical-symlink containment for every non-default-root file, folder, comment-sidecar, order, and image operation. `NotesSearchService` owns lazy native indexes, recursive watchers, debounced/coalesced refresh state, and disposal per `(workspaceId, rootId)`; composition validates the native capability eagerly, while filesystem builds begin on first service search. Ambiguous watcher events recover with a full native refresh, failures retain the prior complete snapshot, and preference/workspace/server lifecycle events evict the matching scopes |
