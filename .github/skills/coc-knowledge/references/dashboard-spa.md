...
The repo-scoped Dreams tab (`features/dreams/DreamsPanel.tsx`) is a dedicated review surface separate from Work Items. It is visible as a workspace sub-tab and renders a disabled-by-global-flag state when `dreams.enabled` is off, then requires the workspace `preferences.dreams.enabled` opt-in before calling Dreams routes. Once enabled, it lists visible cards by default, supports status filters for hidden lifecycle history, exposes a manual **Run dream now** action, shows run summaries/no-new-dreams states, links source process turn ranges back to the Activity conversation route, and offers only record-level card actions: approve, dismiss, record conversion, and supersede. Conversion recording links the card to an already-created artifact and does not create skills, notes, memory, work items, or source changes.
