...
`ChatStyleSelector` renders a `Style: <label>` chip after Effort in both composers (inline toolbar and compact settings editor agree on Effort-then-Style) when `features.chatStyleSelector` is on, listing Human, Direct, Analytical, and Structured with one-line descriptions and the shared checkmark treatment. It mirrors `EffortTierSelector`'s popover, focus, outside-click, dark-mode, and compact-trigger conventions, but every style is always selectable — there is no per-provider configuration. `useChatStyleSelectorEnabled(apiBase)` resolves the flag against the server that owns the target: with no `apiBase` it reads the live dashboard config and subscribes to `DASHBOARD_CONFIG_UPDATED_EVENT`; with one it fetches `${apiBase}/config/runtime` (cached per base) and treats an unreachable or older server as unsupported. `InitialChatComposer` seeds the style from the repo-scoped `lastChatStyle` preference (read from the same request that restores `lastChatProvider`), falls back to Human, persists picks through `preferences.patchRepo`, and only sends `chatStyle` for Ask/Autopilot; a failed preference write still keeps the style for the current send. `ChatDetail` restores the style from `processDetails.metadata.chatStyle` once per process and resets on process change, so conversations stay independent; `useSendMessage.buildMessageRequest()` includes `chatStyle` only when the flag is on. `ConversationMetadataPopover` shows a `Style` row for conversations that stored one. There is no keyboard shortcut for Style.
...
Quick Ask side-notes are gated by the live server flag `features.quickAskSidenotes`. In chat, selecting text in an assistant turn's `MarkdownView` raises a floating pill (`Cmd/Ctrl+J` alternative); the one-shot answer attaches a 💡 bubble to the message's collected Side notes row and opens a compact Copy / Retry / Dismiss popover. Components live under `features/chat/quick-ask/` (`useQuickAskSidenotes`, the selection and anchoring helpers, `QuickAskPill`, `QuickAskSidenotePopover`, and `QuickAskTurnLayer`), with data plumbed from `ChatDetail` through `ConversationArea` to `ConversationTurnBubble`. `QuickAskPill` is a split pill: **✨ Ask AI** (expands into the inline question input) and, when `QuickAskTurnLayer` receives `onAttachContext`, a divider plus **📎 Attach**, which sends the selected text to `useAttachedContext().add(turnIndex, 'assistant', snippet)` and dismisses the pill. Because the whole layer is mounted only under `features.quickAskSidenotes`, the Attach pill is hidden when that flag is off; the right-click **Attach as context** menu item is the flag-independent path.
