...
`WorkItemsTab` presents hierarchy mode as two top-level tracker tabs: **Local** and **Remote**. The Local tab passes `tracker=local-only` to the tree endpoint and shows local creation actions for local-only Epic trees. The Remote tab shows synced GitHub and Azure Boards Epic trees with provider filter chips for All, GitHub, and Azure Boards; the All filter merges `tracker=github-backed` and `tracker=azure-boards-backed` tree responses client-side. The Remote import action opens a provider selector that can import either an existing GitHub Epic issue or an Azure Boards Epic work item from the workspace-configured remote project, then the SPA switches to Remote, selects/highlights the imported root Epic row/card, and keeps the provider filter aligned with the imported provider.
