...
Quick Ask side-notes are gated by the compile-time `QUICK_ASK_SIDENOTES` flag in `featureFlags.ts` (disabled by default, so the pill/bubbles/popover are stripped from the bundle when off) plus the live server flag `features.quickAskSidenotes`. When on, selecting text in an assistant turn's `MarkdownView` raises a floating ✨ Ask AI pill (`Cmd/Ctrl+J` alternative); one click runs a cheap one-shot lookup that attaches a 💡 bubble to a collected "Side notes" row at the bottom of that message, click-to-read in a compact popover with Copy / Retry / Dismiss. Components live under `features/chat/quick-ask/` (`useQuickAskSidenotes` hook, `quick-ask-selection` extractor, `QuickAskPill`, `QuickAskSidenotePopover`, `QuickAskTurnLayer`); data is plumbed from `ChatDetail` through `ConversationArea` to the assistant `ConversationTurnBubble` (mirroring `noteEdits`). The hook is a no-op when the flag is off or process/workspace are unknown, so it is always safe to call.
