...
`commit_chat_bindings` stays the routing source of truth, and each commit chat also carries a denormalized `metadata.commitChat = { commitHash, commitMessage? }` snapshot on its process row. The snapshot is written from the validated `ChatPayload.context.commitChat` when the process is created, survives HEAD moves, restarts, forks, and archived bindings, and is what the conversation metadata popover reads. A rebind after amend/rebase updates the bound process's `metadata.commitChat.commitHash` (keeping any saved message) and rolls the binding back if that update fails, so routing and the displayed commit cannot disagree. An idempotent startup repair (`storage/startup-commit-chat-metadata-backfill.ts`) joins bindings to processes — accepting bare or `queue_`-prefixed task IDs, scoped by workspace — and adds the hash only where the field is missing; it never overwrites an existing object and records no commit message. It is a no-op for file-backed stores.
...
- **Pending messages:** `pendingMessages` persisted in process metadata; append atomically via `appendPendingMessage(processId, message)` (read-append-persist under the store write lock) so concurrent follow-ups cannot lose updates — never read-modify-write the array through `updateProcess`
