...
| POST | `/api/queue/pause` | Pause queue processing globally or for a repo selected by `workspace`/`repoId` query. Body may be empty for an indefinite pause, `{ durationHours: 1\|2\|3\|4\|8 }` for a preset timed pause, or `{ until }` for a future timestamp. |
...
| POST | `/api/queue/pause-marker` | Insert a pause marker between queued items. Body `{ afterIndex?, repoId?, durationHours? }`; `durationHours` is optional and must be `1`, `2`, `3`, `4`, or `8` when present. Indefinite markers pause until manual resume; timed markers start their countdown when the executor reaches and consumes the marker. Returns `201 { markerId, afterIndex, durationHours? }`. |
