...
| GET | `/api/origins/:originId/pull-requests/:prId/reviewers` | Fetch reviewers through an explicit concrete clone. Requires `workspaceId` query metadata and accepts optional `repoId`; results are cached in memory for 30 minutes by canonical origin/PR unless `force=true` is supplied. |
...
| DELETE | `/api/origins/:originId/pull-requests/recent-opened/:prNumber` | Remove a recently opened PR entry from the canonical origin (used by the per-entry remove control and by automatic stale-entry cleanup on a confirmed 404). Optional `workspaceId`/`repoId` query metadata is used only for legacy migration. |
| GET | `/api/origins/:originId/pull-requests/coworker-candidates` | Search open PR authors for Team roster coworker candidates through an explicit concrete clone. Requires `workspaceId` query metadata and accepts optional `repoId`; also accepts required `query` (minimum 2 chars), optional `status`, `scope`, `top`, and `includeRoster`. The route calls provider list pagination directly with a bounded page/result cap, does not enrich diff stats, caches provider-backed query results in memory for 2 minutes per clone/query/status/scope, and returns de-duplicated candidates with `id`, `displayName`, optional `login`, optional `email`, optional `avatarUrl`, `prCount`, and `isInRoster` based on the origin-scoped Team roster. |
...
| GET | `/api/origins/:originId/classify-diff` | Poll for a single PR classification result under a canonical origin. Query: `type=pr`, `identifier=<prId>:<headSha>`, optional `workspaceId`/`repoId` metadata for legacy migration. Returns `{ status: 'none'\|'ready'\|'running', result? }`. |
