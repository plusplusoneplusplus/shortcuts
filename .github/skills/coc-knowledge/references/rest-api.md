...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/workspaces/:id/work-items` | List work items. Supports standard field filters plus `tracker=local-only\|github-backed\|azure-boards-backed`, which filters by inherited Epic-rooted tracker identity. |
...
| PATCH | `/api/workspaces/:id/work-items/:itemId` | Update work item. `tracker` metadata is accepted only on root Epic items. Core field edits on Azure Boards-backed mirror items push title, description, status/state, priority, tags, and parent relation changes to Azure before local storage; stale Azure revisions return a conflict instead of overwriting remote changes. Legacy `syncLinks` payloads are rejected. |
...
| POST | `/api/workspaces/:id/work-items/:itemId/sync-from-azure-boards` | Re-pull an Azure Boards-backed root Epic from the workspace-configured Azure Boards project. Azure-owned core fields are overwritten from native Azure Boards fields/relations, newly discovered descendants are created, mirrored descendants missing from the fresh native hierarchy are deleted, and a deleted Azure root work item deletes the local mirror tree. The response includes `warnings[]` entries with `code: "remote-wins-conflict"` when Azure changed remotely while local Azure-owned fields diverged from the last synced mirror fingerprint. |
...
| GET | `/api/workspaces/:id/work-items/tree` | Read the hierarchy tree. Supports `tracker=local-only\|github-backed\|azure-boards-backed`; descendants inherit the tracker identity of their root Epic. |
