...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/repos/:repoId/pull-requests` | List pull requests; accepts optional `workspaceId` for Team roster behavior, rows are enriched with `diffStats` (`additions`, `deletions`, `changedFiles`) when the provider exposes PR diffs, include `fetchedAt`, and can be served from the active-workspace background-warmed in-memory server cache for 60 minutes unless `force=true` is supplied. List rows and diff stats are cached in memory by resolved canonical origin, status/scope, PR id, and `headSha` when present; no diff contents are durably stored for stats. When Pull Requests, focused diff, and `pullRequests.autoClassifyTeam` are enabled, loaded open Team PRs with `headSha` opportunistically enqueue missing low-priority diff classifications using the existing classify-diff pipeline and the origin-scoped Team roster. |
| GET | `/api/repos/:repoId/pull-requests/:prId` | Fetch and cache PR detail in memory for 10 minutes by resolved canonical origin and PR id, including provider SHA fields (`baseSha`, `headSha`) when available. `force=true` refreshes only this PR for the resolved origin and invalidates its detail, subresource, provider combined diff, and diff-stats cache entries. |
| GET | `/api/repos/:repoId/pull-requests/:prId/threads` | Fetch comment threads; results are cached in memory for 10 minutes per resolved canonical origin/PR. |
| GET | `/api/repos/:repoId/pull-requests/:prId/reviewers` | Fetch reviewers; results are cached in memory for 30 minutes per resolved canonical origin/PR. |
| GET | `/api/repos/:repoId/pull-requests/:prId/commits` | Fetch PR commits; results are cached in memory for 30 minutes per resolved canonical origin/PR. |
| GET | `/api/repos/:repoId/pull-requests/:prId/checks` | Fetch CI/check statuses; results are cached in memory for 10 minutes per resolved canonical origin/PR. |
| GET | `/api/repos/:repoId/pull-requests/:prId/diff` | Return a plain-text provider unified diff for a PR. The provider combined diff is cached in memory with no TTL by resolved canonical origin, PR id, and resolved PR `headSha` when available; if the head SHA cannot be resolved, the server falls back to the origin/PR key and `force=true` on PR detail invalidates that PR's diff entries for the resolved origin. Diff contents are not durably stored. |
| GET | `/api/repos/:repoId/pull-requests/:prId/diff/files/:path` | Return JSON `{ diff }` by extracting one file from the origin-scoped shared in-memory provider combined diff cache used by `/diff`; with `fullContext=true`, the server resolves PR detail, attempts a non-durable local `git diff -U99999` from `baseSha` to `headSha`, may fetch missing commits into the requested repo checkout without checking out branches, and falls back to the provider hunk diff with `fullContextUnavailable: true` metadata. |
| GET | `/api/repos/:repoId/pull-requests/recent-opened` | List recently opened PR entries for the resolved canonical origin (`workspaceId` query selects the concrete workspace and defaults to `repoId`); legacy workspace/repo files migrate into the origin list on access |
| POST | `/api/repos/:repoId/pull-requests/recent-opened` | Record a recently opened PR entry under the resolved canonical origin after successful validation/open; body includes `workspaceId`, `number`, `title`, optional `webUrl` |
| DELETE | `/api/repos/:repoId/pull-requests/recent-opened/:prNumber` | Remove a stale recently opened PR entry from the resolved canonical origin (`workspaceId` query selects the concrete workspace and defaults to `repoId`) |
...
| GET | `/api/repos/:repoId/pull-requests/review-history` | Read cached PR review history from the resolved canonical origin (`workspaceId` query selects the concrete workspace and defaults to `repoId`); legacy workspace/repo cache files migrate into the origin file on access |
| POST | `/api/repos/:repoId/pull-requests/review-history/refresh` | Fetch provider review history for the concrete repo and cache it under the resolved canonical origin (`workspaceId` query selects the concrete workspace and defaults to `repoId`) |
...
| POST | `/api/workspaces/:id/work-item-chat-bindings/:workItemId/fresh` | Archive the currently bound Work Item chat process and clear the binding so the same workspace/Work Item target starts from an empty chat on the next send; stale bindings whose process is already missing are cleared and return `archivedTaskId: null` |
