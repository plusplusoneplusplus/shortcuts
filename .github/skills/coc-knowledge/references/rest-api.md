...
| GET | `/api/queue/models` | List model IDs for the resolved concrete default provider; when `defaultProvider: auto` is enabled, this uses the same Auto provider routing resolver as enqueue and includes `autoProviderRouting` metadata with the selected provider, fallback state, warnings, and decision reasons. |
