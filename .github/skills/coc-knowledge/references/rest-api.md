...
| GET | `/api/workspaces/:id/files/preview?path=<path>` | Read a bounded text/image/directory preview. Relative paths anchor at the workspace root. A repo-group workspace also accepts absolute paths inside live registered member roots; removed or missing-path members are skipped. Non-group scope and all write routes remain workspace-scoped |
...
| POST | `/api/processes/:id/message` | Follow-up message. Body: `content`, optional `mode` (`ask`\|`autopilot`, `plan` accepted as Ask), `deliveryMode`, `images`, `skillNames`, `model`, per-turn `reasoningEffort` (`low`\|`medium`\|`high`\|`xhigh`), and `chatStyle` (`human`\|`direct`\|`analytical`\|`structured`). An unknown `chatStyle` is rejected `400` (an unknown `reasoningEffort` is dropped). A style differing from `metadata.chatStyle` is never steered into an in-flight response — it is buffered as the next pending turn so that turn gets a freshly built system message — and becomes the conversation style thereafter. A `cancelled` process is accepted only with a saved `sdkSessionId`; the continuation binds to that session for strict resume, else `409 SESSION_NOT_RESUMABLE`. `metadata.stoppedChatResume.resumable === false` also returns `409 SESSION_NOT_RESUMABLE`, so a failed strict resume cannot be retried or converted to a fresh session |
...
| POST | `/api/queue` | Enqueue a task. Chat payloads use `mode='ask'`, `mode='autopilot'`, or internal Ralph routing (`mode='plan'` normalizes to Ask). For Each item-plan generation is a normal Ask chat with `payload.context.forEach.kind='generation'`; the UI-only `for-each` mode value is rejected by the generic validator. Optional `payload.chatStyle` (`human`\|`direct`\|`analytical`\|`structured`) is rejected `400` when unknown and copied into `process.metadata.chatStyle` at execution start. `config.effortTier` follows [Provider overrides](#provider-overrides). Notes chat edits may carry `payload.context.lensChat = { inherited: true, source: 'features.commitChatLens' }` when the Lens Chat flag is active; the marker is copied to process metadata |
