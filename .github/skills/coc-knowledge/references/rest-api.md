...
| GET/POST | `/api/workspaces/:id/work-items/...` | Workspace-compatible Work Item routes resolve to the workspace's canonical origin for storage/cache reads and writes while using `:id` as the concrete workspace. New SPA and `coc-client` callers use the origin routes directly. |
