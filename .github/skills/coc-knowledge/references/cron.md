...
3. A Sentinel process reads `notes/Sentinel/Sentinel.md`, applies its tick interval, recency window, mute list, and nudge budget, then runs its workspace classifier once inside the in-flight guard before enqueue. It folds board edits, publishes the regenerated board with optimistic-write retries, and executes only newly checked approval rows. The classifier batches final completed-chat turns into one tool-free model transform; a scan, configuration, board-conflict, model-validation, or approved-nudge failure counts as a failed tick and does not enqueue the Sentinel follow-up. Ordinary crons skip this step.
...
- `POST /api/workspaces/:id/sentinel/check-now` — when Sentinel is enabled, validate the workspace watchlist owner and immediately run its active scan cron through the normal tick guards; returns `404` without a live owner and `409` while busy or without an active scan cron
...
`CronBadge` (header count of non-cancelled crons) and `CronManagementPanel` (list with pause/resume/cancel; each active cron shows its next tick relative to `nextTickAt`, plus tick count and last-tick time) render only when `cronEnabled`. A feature-enabled Sentinel conversation exposes a **Check now** header action that starts its workspace-scoped tick and disables while the request is pending. `ConversationTurnBubble` shows a turn-source indicator for turns generated by a cron tick or wakeup, read from `turnSource` on `ConversationTurn`. The `/cron` slash command auto-installs the `/cron` skill and activates cron tools for the session.

## Server Lifecycle

- **Startup:** if `cron.enabled` or Sentinel is enabled, `CronStore` and `CronExecutor` are constructed. General cron startup restores every active timer; Sentinel-only startup restores only Sentinel timers. A newly admitted Sentinel provisions one hourly process-bound scan cron from the aggregate queue's successful `taskAdded` event. Confirmed Sentinel replacement cancels and disarms the old process-bound scan cron while retaining the old conversation. The Sentinel-specific check-now route remains available without enabling general cron routes.
