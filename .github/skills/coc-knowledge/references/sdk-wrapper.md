...
| `effort-tier-defaults.ts` | Per-provider effort-tier defaults (`very-low`/`low`/`medium`/`high` → model + reasoning effort) + stored-config merge. Codex uses `gpt-6-luna` for Very Low, `gpt-6-sol` for Medium, and `gpt-6-astra` for High. Claude uses CLI aliases `haiku`/`sonnet`/`opus`/`fable` (`high` → `fable`), with no pinned effort for Haiku. |
