...
| `type` | `"work-item" \| "bug"` | | Defaults to `"work-item"`. Use `"bug"` for bug reports. |
...
**Response: 201** — Full `WorkItem` object including `id`, `status: "created"`, `type`.

---

## List Work Items (with type filter)

```
GET /api/workspaces/:workspaceId/work-items?type=bug
```
