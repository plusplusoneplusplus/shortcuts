...
export function getTaskTypeIcon(task: any): string {
    ...
    if (payload.scheduleId || task.scheduleId) return '≡ƒôà';
    if (type === 'chat') {
        if (mode === 'ask') return '≡ƒÆí';
...
export interface ActivityListPaneProps {
    ...
    /** Callback when user types in search ΓÇö drives server-side search from parent. */
...
export function ActivityListPane({
    ...
    // ΓöÇΓöÇ History/archived long-press via shared useLongPress hook ΓöÇΓöÇ
    ...
    const contextMenuItems = useMemo<ContextMenuItem[]>(() => {
        ...
        if (contextMenu.bulkIds) {
            ...
            return [
                ...
                ...(ids.length === 1 ? [{
                    label: 'Rename', icon: 'Γ£Å∩╕Å', onClick: () => {
                ...
                <div className="flex items-center justify-between gap-1.5 text-xs text-[#1e1e1e] dark:text-[#cccccc]">
                    <span className="flex items-center gap-1 min-w-0 truncate">
                        ...
                        <span className="shrink-0">
                            {getTaskTypeIcon(task)}{task.status === 'completed' ? ' Γ£à' : task.status === 'failed' ? ' Γ¥î' : task.status === 'cancelled' ? ' ≡ƒÜ½' : ''}
                        </span>
                        ...
                        {hasDraft && <span className="shrink-0 text-[10px] text-[#848484] dark:text-[#bbb]" title="Unsent draft" data-testid="draft-badge">Γ£Å∩╕Å</span>}
                    </span>
        ...
        <>
            <div className="p-4 flex flex-col gap-3 overflow-y-auto flex-1">
                ...
                {searchVisible && (
                    <div className="flex items-center gap-1.5 px-1 py-1 rounded border border-[#e0e0e0] dark:border-[#474749] bg-[#fafafa] dark:bg-[#1e1e1e] text-xs">
                        ...
                        <input
                            ref={searchInputRef}
                            type="text"
                            placeholder="Search all conversationsΓÇª"
                            value={searchQuery}
                        ...
                        />
                        {searchLoading && (
                            <span className="text-[#848484] animate-pulse" data-testid="search-loading-indicator">ΓÅ│</span>
                        )}
                ...
                {filteredRunning.length > 0 && (
                    <div>
                        ...
                        >
                            {showRunning ? 'Γû╝' : 'Γû╢'} Running Tasks <span className="text-[10px]">({filteredRunning.length})</span>
                        </button>
                        {showRunning && (
                ...
                {filteredQueued.length > 0 && (
                    <div>
                        ...
                        >
                            {showQueued ? 'Γû╝' : 'Γû╢'} Queued Tasks <span className="text-[10px]">({filteredQueued.filter((t: any) => t.kind !== 'pause-marker').length})</span>
                        </button>
                        {showQueued && (
                            <div className={cn("flex flex-col", isDense ? "gap-0.5" : "gap-1")}>
                                {!isMobile && (
                ...
                {isServerSearchActive ? (
                    /* ΓöÇΓöÇ Server-side search results ΓöÇΓöÇ */
                    <div>
                        ...
                        {searchHasMore && onLoadMoreSearchResults && (
                            <div className="px-4 py-2">
                                ...
                                >
                                    {searchLoadingMore ? 'LoadingΓÇª' : 'Load more results'}
                                </button>
                            </div>
                        )}
                    </div>
                ) : (
                    /* ΓöÇΓöÇ Normal history view (pinned + unpinned + archived + load more) ΓöÇΓöÇ */
                    <>
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-1.5 text-xs text-[#1e1e1e] dark:text-[#cccccc]">
                                                <span className="flex items-center gap-1 min-w-0 truncate">
                                                    ...
                                                    {hasPinnedDraft && <span className="shrink-0 text-[10px] text-[#848484] dark:text-[#bbb]" title="Unsent draft" data-testid="draft-badge">Γ£Å∩╕Å</span>}
                                                </span>
                                                <span className="text-[10px] text-[#848484] dark:text-[#bbb] shrink-0 whitespace-nowrap tabular-nums">
                                                    {(task.completedAt ?? task.endTime) ? formatRelativeTime(new Date(task.completedAt ?? task.endTime).toISOString()) : ''}
                                                </span>
            ...
            {hasMore && onLoadMore && (
                <div className="px-4 py-2">
                    ...
                    >
                        {loadingMore ? 'LoadingΓÇª' : 'Load more'}
                    </button>
                </div>
            )}
                    </>
        ...
        >
            <div className="flex items-center justify-between gap-1.5">
                <div className="flex items-center gap-1.5 text-xs text-[#1e1e1e] dark:text-[#cccccc] min-w-0">
                    <span className="shrink-0">{task.frozen ? 'Γ¥ä∩╕Å' : isAdmitted ? '≡ƒÜÇ' : isHeld ? '≡ƒñûΓÅ╕' : icon}</span>
