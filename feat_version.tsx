/**
 * ActivityListPane ΓÇö shared queue-style left rail for Activity and Queue tabs.
 *
...
export const SESSION_CATEGORY_LABELS: Record<string, { label: string; icon: string; color: string }> = {
    'generating-code': { label: 'Generating Code', icon: 'ΓÜÖ∩╕Å', color: 'text-blue-600 dark:text-blue-400' },
    'resolve-plan-comments': { label: 'Resolve Plan', icon: '≡ƒô¥', color: 'text-purple-600 dark:text-purple-400' },
    'resolve-commit-comments': { label: 'Resolve Commit', icon: '≡ƒöº', color: 'text-amber-600 dark:text-amber-400' },
};
...
export function getTaskPromptPreview(task: any): string {
    ...
    return text.length > 60 ? text.substring(0, 57) + 'ΓÇª' : text;
}
...
export function ActivityListPane({
    ...
    const chatAllItems = useMemo(() => {
        ...
        // Deduplicate by processId ΓÇö running tasks take priority (appear first)
    ...
    // ΓöÇΓöÇ History/archived long-press (shared refs ΓÇö only one touch at a time) ΓöÇΓöÇ
    ...
    const contextMenuItems = useMemo<ContextMenuItem[]>(() => {
        ...
        if (contextMenu.bulkIds) {
            ...
            return [
                ...
                ...(anyUnseen && onMarkRead    ? [{ label: 'Mark as Read',   icon: 'Γ£ô', onClick: () => { ids.forEach(id => onMarkRead!(id));   closeContextMenu(); } }] : []),
                ...(anySeen && onMarkUnread    ? [{ label: 'Mark as Unread', icon: 'ΓùÅ', onClick: () => { ids.forEach(id => onMarkUnread!(id)); closeContextMenu(); } }] : []),
                ...(anyPinned && onUnpinChat   ? [{ label: 'Unpin',          icon: '≡ƒôî', onClick: () => { ids.forEach(id => onUnpinChat!(id)); closeContextMenu(); } }] : []),
                ...(anyUnpinned && onPinChat   ? [{ label: 'Pin to top',     icon: '≡ƒôî', onClick: () => { ids.forEach(id => onPinChat!(id));   closeContextMenu(); } }] : []),
                ...(anyUnarchived && onArchiveChats  ? [{ label: 'Archive',   icon: '≡ƒôª', onClick: () => { onArchiveChats!(ids);   closeContextMenu(); } }] : []),
                ...(anyArchived  && onUnarchiveChats ? [{ label: 'Unarchive', icon: '≡ƒôñ', onClick: () => { onUnarchiveChats!(ids); closeContextMenu(); } }] : []),
                ...(ids.length <= 20 ? [{
                ...
                { label: '', icon: '', separator: true, onClick: () => {} },
                { label: `Delete ${ids.length} chatsΓÇª`, icon: '≡ƒùæ', onClick: () => {
        ...
        if (taskStatus === 'running') {
            ...
            return [
                ...(isPinned && onUnpinChat ? [{ label: 'Unpin', icon: '≡ƒôî', onClick: () => onUnpinChat(taskId) }] : []),
                ...(!isPinned && onPinChat ? [{ label: 'Pin to top', icon: '≡ƒôî', onClick: () => onPinChat(taskId) }] : []),
                { label: 'Copy metadata', icon: '≡ƒôï', onClick: () => {
                    const task = running.find((t: any) => t.id === taskId);
                    if (task) void copyToClipboard(formatMetadataText(task));
                    closeContextMenu();
                }},
                { label: '', icon: '', separator: true, onClick: () => {} },
                { label: 'Cancel', icon: 'Γ£ò', onClick: () => handleCancel(taskId) },
            ];
        }
        if (taskStatus === 'completed') {
            ...
            return [
                ...(isPinned && onUnpinChat ? [{ label: 'Unpin', icon: '≡ƒôî', onClick: () => onUnpinChat(taskId) }] : []),
                ...(!isPinned && onPinChat ? [{ label: 'Pin to top', icon: '≡ƒôî', onClick: () => onPinChat(taskId) }] : []),
                ...(isUnseen && onMarkRead ? [{ label: 'Mark as Read', icon: 'Γ£ô', onClick: () => onMarkRead(taskId) }] : []),
                ...(!isUnseen && onMarkUnread ? [{ label: 'Mark as Unread', icon: 'ΓùÅ', onClick: () => onMarkUnread(taskId) }] : []),
                ...(isArchived && onUnarchiveChat ? [{ label: 'Unarchive', icon: '≡ƒôñ', onClick: () => onUnarchiveChat(taskId) }] : []),
                ...(!isArchived && onArchiveChat ? [{ label: 'Archive', icon: '≡ƒôª', onClick: () => onArchiveChat(taskId) }] : []),
                { label: '', icon: '', separator: true, onClick: () => {} },
                { label: 'Delete chat', icon: '≡ƒùæ', onClick: () => handleDeleteChat(taskId) },
            ];
        }
        ...
        return [
            ...(queuedIndex > 0 ? [{ label: 'Move Up', icon: 'Γû▓', onClick: () => handleMoveUp(taskId) }] : []),
            { label: 'Move to Top', icon: 'ΓÅ¼', onClick: () => handleMoveToTop(taskId) },
            { label: '', icon: '', separator: true, onClick: () => {} },
            ...(isHeld ? [{ label: 'Schedule Immediately', icon: '≡ƒÜÇ', onClick: () => handleAdmit(taskId) }] : []),
            ...(isAdmitted ? [{ label: 'Cancel Scheduling', icon: '≡ƒÜ½', onClick: () => handleUnadmit(taskId) }] : []),
            ...
            isFrozen
                ? { label: 'Unfreeze', icon: 'Γû╢', onClick: () => handleUnfreeze(taskId) }
                : { label: 'Freeze', icon: 'Γ¥ä', onClick: () => handleFreeze(taskId) },
            { label: 'Cancel', icon: 'Γ£ò', onClick: () => handleCancel(taskId) },
        ];
        ...
        return (
            <div className="relative flex-1 flex flex-col overflow-hidden">
                <div className="p-4 text-center text-sm text-[#848484] flex-1" data-testid="queue-empty-state">
                    {isRefreshing && (
                        <div className="mb-2 animate-pulse" data-testid="queue-refreshing-indicator">RefreshingΓÇª</div>
                    )}
                    {activeTab === 'chats' ? (
        ...
        <div className="relative flex-1 flex flex-col overflow-hidden">
            <div className="p-4 flex flex-col gap-3 overflow-y-auto flex-1">
                ...
                {activeTab === 'tasks' && isPaused && (
                    <div className="rounded bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 px-3 py-1.5 text-xs flex items-center gap-2" data-testid="queue-paused-banner">
                        <span className="flex-1">
                            {pauseReason
                                ? <>ΓÅ╕ Queue paused ΓÇö <strong>{pauseReason.displayName}</strong> failed at {new Date(pauseReason.failedAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}.</>
                                : <>ΓÅ╕ Queue is paused ΓÇö new tasks will not start.</>
                            }
                        </span>
                        {pauseReason && (
                            <Button
                ...
                {activeTab === 'tasks' && isAutopilotPaused && (
                    ...
                    >
                        <span className="flex-1">≡ƒñûΓÅ╕ Autopilot is paused ΓÇö queued autopilot tasks will not start.</span>
                        <Button
                            variant="ghost"
                            size="sm"
                ...
                {(activeTab === 'tasks' ? searchVisible : true) && (
                    <div className="flex items-center gap-1.5 px-1 py-1 rounded border border-[#e0e0e0] dark:border-[#474749] bg-[#fafafa] dark:bg-[#1e1e1e] text-xs">
                        <span className="text-[#848484]">≡ƒöì</span>
                        <input
                            ref={searchInputRef}
                            type="text"
                            placeholder="Search title, promptΓÇª"
                            value={searchQuery}
                ...
                {tabFilteredRunning.length > 0 && (
                    <div>
                        ...
                        >
                            {showRunning ? 'Γû╝' : 'Γû╢'} Running Tasks <span className="text-[10px]">({tabFilteredRunning.length})</span>
                        </button>
                        {showRunning && (
                ...
                {tabFilteredQueued.length > 0 && (
                    <div>
                        ...
                        >
                            {showQueued ? 'Γû╝' : 'Γû╢'} Queued Tasks <span className="text-[10px]">({tabFilteredQueued.filter((t: any) => t.kind !== 'pause-marker').length})</span>
                        </button>
                        {showQueued && (
                            <div className="flex flex-col gap-1">
                                {!isMobile && (
                ...
                {(filteredPinned.length > 0 || pinnedRunningCount > 0) && (
                    <div>
                        ...
                        >
                            {showPinned ? 'Γû╝' : 'Γû╢'} ≡ƒôî Pinned ({filteredPinned.length + pinnedRunningCount})
                        </button>
                        {showPinned && (
                            <div className="flex flex-col gap-1">
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-1.5 text-xs">
                                                <span className="flex items-center gap-1 min-w-0 truncate">
                                                    {isHistorySelected && <span className="shrink-0 text-[#0078d4] dark:text-[#3794ff] text-[10px]" data-testid="selection-checkbox">Γÿæ</span>}
                                                    {isUnseen && <span className="shrink-0 w-1.5 h-1.5 rounded-full bg-[#0078d4] dark:bg-[#3794ff]" data-testid="unseen-dot" />}
                                                    {task.status === 'failed' && <span className="shrink-0">Γ¥î</span>}
                                                    <span className={cn("truncate", isUnseen && "font-semibold")} title={task.displayName || task.type || 'Task'}>
                                                        {task.displayName || task.type || 'Task'}
                                                    </span>
                                                    {hasPinnedDraft && <span className="shrink-0 text-[10px] text-[#848484] dark:text-[#999]" title="Unsent draft" data-testid="draft-badge">Γ£Å∩╕Å</span>}
                ...
                {filteredUnpinned.length > 0 && (
                    <div>
                        <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            >
                                {showHistory ? 'Γû╝' : 'Γû╢'} Completed Tasks ({filteredUnpinned.length})
                                {unseenTaskIds && (() => {
                            ...
                            {selectedHistoryIds.size >= 2 && (
                                <span className="inline-flex items-center gap-1 text-[10px] bg-[#0078d4]/15 text-[#0078d4] dark:bg-[#3794ff]/15 dark:text-[#3794ff] px-2 py-0.5 rounded-full" data-testid="selection-count-pill">
                                    ...
                                    <button className="leading-none hover:text-red-500" onClick={() => { setSelectedHistoryIds(new Set()); setAnchorHistoryId(null); }} data-testid="selection-clear-btn">Γ£ò</button>
                                </span>
                            )}
                        </div>
                        {showHistory && (
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-1.5 text-xs">
                                                <span className="flex items-center gap-1 min-w-0 truncate">
                                                    ...
                                                    {hasUnpinnedDraft && <span className="shrink-0 text-[10px] text-[#848484] dark:text-[#999]" title="Unsent draft" data-testid="draft-badge">Γ£Å∩╕Å</span>}
                                                    {(() => { const cat = getSessionCategory(task); const m = cat ? SESSION_CATEGORY_LABELS[cat] : undefined; return m ? <span className={cn("shrink-0 text-[10px] font-medium", m.color)} data-testid="session-category-badge">{m.icon}</span> : null; })()}
                                                </span>
                                                <span className="text-[10px] text-[#848484] dark:text-[#999] shrink-0 whitespace-nowrap tabular-nums">
                                                    {task.completedAt ? formatRelativeTime(new Date(task.completedAt).toISOString()) : ''}
            ...
            {filteredArchived.length > 0 && (
                <div>
                    ...
                    >
                        {showArchived ? 'Γû╝' : 'Γû╢'} ≡ƒôª Archived ({filteredArchived.length})
                    </button>
                    {showArchived && (
                        <div className="flex flex-col gap-1">
                ...
                {activeTab === 'chats' && (<>
                    {chatAllItems.pinned.length > 0 && (
                        <div>
                            <div className="text-[11px] uppercase text-[#848484] dark:text-[#a0a0a0] font-medium mb-1">≡ƒôî Pinned</div>
                            <div className="flex flex-col gap-1">
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-1.5 text-xs">
                                                <span className="flex items-center gap-1 min-w-0 truncate">
                                                    ...
                                                    {!isRunning && task.status === 'failed' && <span className="shrink-0">Γ¥î</span>}
                                                    <span className={cn('truncate', isUnseen && 'font-semibold')} title={task.displayName || 'Chat'}>{task.displayName || 'Chat'}</span>
                                                    {hasDraft && <span className="shrink-0 text-[10px] text-[#848484]" title="Unsent draft">Γ£Å∩╕Å</span>}
                                                    {(() => { const cat = getSessionCategory(task); const m = cat ? SESSION_CATEGORY_LABELS[cat] : undefined; return m ? <span className={cn("shrink-0 text-[10px] font-medium", m.color)} data-testid="session-category-badge">{m.icon}</span> : null; })()}
                                                </span>
                                                <span className="text-[10px] text-[#848484] dark:text-[#999] shrink-0 whitespace-nowrap tabular-nums">
                    ...
                    {chatAllItems.unpinned.length > 0 && (
                        <div>
                            <div className="text-[11px] uppercase text-[#848484] dark:text-[#a0a0a0] font-medium mb-1">≡ƒòÉ Recently</div>
                            <div className="flex flex-col gap-1">
                    ...
                    {chatAllItems.archived.length > 0 && (
                        <div>
                            ...
                            >
                                {showArchived ? 'Γû╝' : 'Γû╢'} ≡ƒôª Archived ({chatAllItems.archived.length})
                            </button>
                            {showArchived && (
                                <div className="flex flex-col gap-1">
        ...
        >
            <div className="flex items-center justify-between gap-1.5">
                <div className="flex items-center gap-1.5 text-xs text-[#1e1e1e] dark:text-[#cccccc] min-w-0">
                    <span className="shrink-0">{task.frozen ? 'Γ¥ä∩╕Å' : isAdmitted ? '≡ƒÜÇ' : isHeld ? '≡ƒñûΓÅ╕' : ''}</span>
                    <span className="truncate" title={name}>{name}</span>
                    {isPinned && <span className="shrink-0 text-[10px]" data-testid="running-pin-badge">≡ƒôî</span>}
                    {isHeld && (
                        <span
                    ...
                    {hasDraft && <span className="shrink-0 text-[10px] text-[#848484] dark:text-[#999]" title="Unsent draft" data-testid="draft-badge">Γ£Å∩╕Å</span>}
                    {(() => {
                        const cat = getSessionCategory(task);
            ...
            {showProgress && progress && progress.total > 0 && (
                <div className="mt-1" data-testid="workflow-progress-indicator">
                    <div className="text-[10px] text-[#0078d4] dark:text-[#3794ff]">
                        Γû╢ Map: {progress.completed}/{progress.total}
                    </div>
...
function PauseMarkerRow({ markerId, onRemove }: { markerId: string; onRemove: () => void }) {
    return (
        ...
        >
            <span className="shrink-0 text-[11px]">ΓÅ╕</span>
