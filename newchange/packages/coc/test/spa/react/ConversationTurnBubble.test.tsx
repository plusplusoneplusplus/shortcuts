...
describe('ConversationTurnBubble — copy button visibility', () => {
    beforeEach(() => {
        vi.restoreAllMocks();
    });

    it('copy button is always visible (no opacity-0 class) on assistant messages', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Hello' })} />);
        const btn = container.querySelector('.bubble-copy-btn') as HTMLElement;
        expect(btn).toBeTruthy();
        expect(btn.className).not.toContain('opacity-0');
        expect(btn.className).not.toContain('group-hover');
    });

    it('copy button is always visible (no opacity-0 class) on user messages', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'user', content: 'Hi' })} />);
        const btn = container.querySelector('.bubble-copy-btn') as HTMLElement;
        expect(btn).toBeTruthy();
        expect(btn.className).not.toContain('opacity-0');
        expect(btn.className).not.toContain('group-hover');
    ...
    it('copy button is always visible on error messages', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Error occurred', isError: true })} />);
        const btn = container.querySelector('.bubble-copy-btn') as HTMLElement;
        expect(btn).toBeTruthy();
        expect(btn.className).not.toContain('opacity-0');
    });

    it('copy button is always visible on streaming messages', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Partial response...', streaming: true })} />);
        const btn = container.querySelector('.bubble-copy-btn') as HTMLElement;
        expect(btn).toBeTruthy();
        expect(btn.className).not.toContain('opacity-0');
    });

    it('HTML copy button is always visible (no opacity-0 class) on assistant messages', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Test' })} />);
        const btn = container.querySelector('.bubble-copy-html-btn') as HTMLElement;
        expect(btn).toBeTruthy();
        expect(btn.className).not.toContain('opacity-0');
        expect(btn.className).not.toContain('group-hover');
    });

    it('raw toggle button still uses hover-only visibility', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Test' })} />);
        const btn = container.querySelector('.bubble-raw-btn') as HTMLElement;
        expect(btn).toBeTruthy();
...
describe('ConversationTurnBubble — HTML copy button', () => {
    beforeEach(() => {
        vi.restoreAllMocks();
        vi.useFakeTimers();
    });
    ...
    it('renders .bubble-copy-html-btn for assistant messages', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Hello' })} />);
        const btn = container.querySelector('.bubble-copy-html-btn');
        expect(btn).toBeTruthy();
    });

    it('renders .bubble-copy-html-btn for user messages', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'user', content: 'Hello' })} />);
        const btn = container.querySelector('.bubble-copy-html-btn');
        expect(btn).toBeTruthy();
    });

    it('calls copyHtmlToClipboard when HTML copy button is clicked', async () => {
        const spy = vi.spyOn(formatUtils, 'copyHtmlToClipboard').mockResolvedValue(undefined);

        const { container } = render(
            <ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Copy me as HTML!' })} />
        );
        const btn = container.querySelector('.bubble-copy-html-btn') as HTMLButtonElement;
        await act(async () => { fireEvent.click(btn); });
        expect(spy).toHaveBeenCalledTimes(1);
        // Should have been called with HTML-rendered content
        expect(spy).toHaveBeenCalledWith(expect.any(String));
    });

    it('shows ✓ after successful HTML copy and resets after 1.5s', async () => {
        vi.spyOn(formatUtils, 'copyHtmlToClipboard').mockResolvedValue(undefined);

        const { container } = render(
            <ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Hello' })} />
        );
        const btn = container.querySelector('.bubble-copy-html-btn') as HTMLButtonElement;
        expect(btn.textContent).toBe('HTML');
    ...
    it('keeps HTML label when HTML copy fails', async () => {
        vi.spyOn(formatUtils, 'copyHtmlToClipboard').mockRejectedValue(new Error('denied'));
        const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
        ...
        await act(async () => { fireEvent.click(btn); });
        expect(btn.textContent).toBe('HTML');
        expect(consoleSpy).toHaveBeenCalledWith('Copy HTML failed:', expect.any(Error));
    });

    it('has title attribute for accessibility', () => {
        const { container } = render(<ConversationTurnBubble turn={makeTurn({ role: 'assistant', content: 'Test' })} />);
        const btn = container.querySelector('.bubble-copy-html-btn') as HTMLElement;
        expect(btn.getAttribute('title')).toBe('Copy as HTML');
    });
});
